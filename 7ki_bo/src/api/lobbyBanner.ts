...
export interface LobbyBannerUpdateRequest extends Partial<LobbyBannerCreateRequest> {
  id: number;
}
...
// API methods
export const getBannerList = async (params?: LobbyBannerFilters): Promise<LobbyBannerListResponse> => {
  const response = await requestClient.get<LobbyBannerListResponse>('/banners', { params });
  return response;
};

export const createBanner = async (data: LobbyBannerCreateRequest): Promise<LobbyBanner> => {
  const response = await requestClient.post<LobbyBanner>('/banners', data);
  return response;
};

export const updateBanner = async (id: number, data: Partial<LobbyBannerCreateRequest>): Promise<LobbyBanner> => {
...
export const duplicateBanner = async (id: number): Promise<LobbyBanner> => {
  const response = await requestClient.post<LobbyBanner>(`/banners/${id}/duplicate`);
  return response;
};

export const toggleBannerStatus = async (id: number, status: 'active' | 'inactive' | 'draft'): Promise<LobbyBanner> => {
  const response = await requestClient.put<LobbyBanner>(`/banners/${id}/status`, { status });
  return response;
};
...
export const getStatusType = (status: string): 'success' | 'error' | 'warning' => {
