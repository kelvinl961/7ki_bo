...
import { useAccessStore, resetAllStores } from '@vben/stores';
...
function createRequestClient(baseURL: string, options?: RequestClientOptions) {
  ...
  client.addRequestInterceptor({
    fulfilled: async (config) => {
      ...
      const isDomainManagement = config.url?.includes('/domain-management') || 
      ...
      if (config.method?.toUpperCase() === 'GET' && !config.params?.forceRefresh) {
  ...
  client.addResponseInterceptor({
    fulfilled: async (response) => {
      ...
      const config = (response as any).config || (response as any).request?.config;
      
      // ✅ IndexedDB Cache: Cache successful GET responses
      if (config?.__checkCache && config?.method?.toUpperCase() === 'GET' && data) {
        ...
        } else if (url.includes('/reports/') || url.includes('/statistics/') || url.includes('/operations-statistics/')) {
      ...
      if (data && typeof data === 'object') {
        ...
        if (data.list && data.pagination && (data.pageTotals || data.overallTotals)) {
          return data; // Return the whole object including pagination
        }
...
export const requestClient = createRequestClient(apiURL, {
  ...
  timeout: 600000, // 10 minutes for large file imports
});
