        ...
        <div v-else class="space-y-3">
          ...
          <div class="mb-3">
            <n-tabs type="segment" size="medium" animated>
              <n-tab-pane name="all" tab="全部">
                ...
                <div class="grid grid-cols-2 gap-2 max-h-80 overflow-y-auto">
                  <div
                    ...
                    :class="{
                      'bg-blue-50 border-blue-200': isPlatformSelected(platform.platformId),
                      'bg-gray-50 border-gray-200': !isPlatformSelected(platform.platformId)
                    }"
                  >
                    ...
                    <div class="flex items-center justify-between mb-1">
                      <div class="flex items-center space-x-2">
                        <n-checkbox
                          ...
                          @update:checked="(checked) => togglePlatform(platform, checked)"
                          size="small"
                        />
                        ...
                        <div class="flex items-center space-x-1">
                          ...
                          <div class="flex-1 min-w-0">
                            <h4 class="text-sm font-medium text-gray-900">{{ platform.platformName }}</h4>
                            <p class="text-xs text-gray-500">
                              {{ platform.gameType }} • {{ platform.games.length }} 个游戏
                              <n-tag v-if="platform.isFeatured" size="small" type="success" class="ml-1">
                                精选
                              </n-tag>
                              <n-tag v-if="platform.isHot" size="small" type="warning" class="ml-1">
                                热门
                              </n-tag>
                            </p>
                          </div>
                      ...
                      >
                        <n-icon :component="expandedPlatforms.has(platform.platformId) ? ChevronUpOutline : ChevronDownOutline" />
                      </n-button>
                    </div>

                    <!-- Game Selection Options - Ultra Compact -->
                    <div v-if="isPlatformSelected(platform.platformId)" class="ml-7 space-y-1">
                      <!-- Game Selection Mode -->
                      <n-radio-group
                        :value="getGameSelectionMode(platform.platformId)"
                        @update:value="(mode) => updateGameSelectionMode(platform.platformId, mode)"
                        size="small"
                      >
                        <n-space size="small">
                      ...
                      <div
                        v-if="getGameSelectionMode(platform.platformId) === 'specific_games' && expandedPlatforms.has(platform.platformId)"
                        class="mt-1"
                      >
                        ...
                        <div class="max-h-32 overflow-y-auto">
                          <div class="grid grid-cols-2 gap-1">
                            <div
                              ...
                              :key="game.gameId"
                              class="flex items-center space-x-1 p-1 rounded border"
                              :class="{
                                'bg-blue-50 border-blue-200': isGameSelected(platform.platformId, game.gameId),
                                'bg-white border-gray-200': !isGameSelected(platform.platformId, game.gameId)
                              }"
                            >
                              <n-checkbox
                                :checked="isGameSelected(platform.platformId, game.gameId)"
                                @update:checked="(checked) => toggleGame(platform.platformId, game, checked)"
                                size="small"
                              />
                              ...
                              <div class="flex items-center space-x-1 flex-1 min-w-0">
                                ...
                                <div class="min-w-0 flex-1">
                                  <p class="text-xs font-medium text-gray-900 truncate">{{ game.gameName }}</p>
                                </div>
                              </div>
                            </div>
                          </div>
                        ...
                        <div class="mt-1 text-xs text-gray-500">
                          {{ getSelectedGamesCount(platform.platformId) }}/{{ platform.games.length }}
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <n-tab-pane v-for="gameType in gameTypes" :key="gameType.value" :name="gameType.value" :tab="gameType.label">
          ...
          <!-- Selection Summary -->
          <div v-if="selectedPlatforms.length > 0" class="mt-3 p-2 bg-blue-50 rounded text-xs">
...
import { GameControllerOutline, ChevronUpOutline, ChevronDownOutline } from '@vicons/ionicons5';
import { 
...
interface Props {
  ...
  wageringPlatform?: 'all_platforms' | 'specific_platforms' | 'exclude_platforms';
  disabled?: boolean;
}
...
const totalSelectedGames = computed(() => {
  return selectedPlatforms.value.reduce((total, platform) => {
    if (platform.gameSelection === 'all_games') {
      const platformData = platforms.value.find(p => p.platformId === platform.platformId);
...
const getGameSelectionMode = (platformId: number): 'all_games' | 'specific_games' => {
  const platform = selectedPlatforms.value.find(p => p.platformId === platformId);
...
const getSelectedGamesCount = (platformId: number): number => {
  ...
  if (platform.gameSelection === 'all_games') {
    const platformData = platforms.value.find(p => p.platformId === platformId);
...
const updateGameSelectionMode = (platformId: number, mode: 'all_games' | 'specific_games') => {
  const platform = selectedPlatforms.value.find(p => p.platformId === platformId);
  if (platform) {
