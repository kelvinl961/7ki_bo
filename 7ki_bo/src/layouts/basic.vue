...
const totalNotificationCount = computed(() => {
  return Object.values(notificationCategories.value).reduce((sum, count) => sum + count, 0);
});
...
const { isWarning: isIdleWarning } = useIdleTimeout({
  ...
  onWarning: () => {
    ...
    dialog.warning({
      title: '⚠️ 非活动会话警告',
      content: '您已经 3 小时 55 分钟没有活动。如果未检测到任何活动，您将在 5 分钟后自动退出登录。',
      positiveText: '我还在这里',
      onPositiveClick: () => {
...
function setupNotificationStream() {
  ...
  console.log('📡 Admin notification stream disabled - manual refresh preferred');
  return;
...
async function fetchOnlineUsersCount() {
  try {
    ...
    console.error('❌ FAILED to fetch online users - using fallback mock data:', error);
  ...
  try {
    ...
    if (result && result.onlineRechargeError !== undefined) {
      notificationCategories.value = {
        ...
        prizeDistributionMonitoringReview: result.prizeDistributionMonitoringReview || 0
      };
      
      console.log('✅ Notification counts updated:', notificationCategories.value);
      
      // Check for new notifications and play sound if needed
      const newTotalCount = Object.values(notificationCategories.value).reduce((sum, count) => sum + count, 0);
      checkForNewNotifications(newTotalCount);
    } else {
      throw new Error('Invalid notification response format');
    }
  } catch (error) {
    console.error('❌ FAILED to fetch notification counts - using fallback data:', error);
...
onMounted(async () => {
  ...
  console.log('🎯 [basic.vue] About to call AdminNotificationService.initialize()...');
  try {
    await AdminNotificationService.initialize();
    console.log('✅ [basic.vue] AdminNotificationService.initialize() completed');
  } catch (error) {
    console.error('❌ [basic.vue] AdminNotificationService.initialize() failed:', error);
  }
  ...
  if (!accessStore.accessToken) {
    const retrySetup = () => {
      if (accessStore.accessToken) {
        console.log('📡 Token became available, setting up notification stream');
        setupNotificationStream();
      } else {
  ...
  <BasicLayout @clear-preferences-and-logout="handleLogout">
    <template #user-dropdown>
      <UserDropdown
        ...
        :description="userStore.userInfo?.email || userStore.userInfo?.username || userStore.userInfo?.realName || 'User'"
        tag-text="Pro"
        @logout="handleLogout"
      />
    </template>
    <template #header-online-users>
      <div class="flex-center mr-2 h-full gap-2">
        ...
        <div 
          ...
          class="flex items-center gap-2 px-3 py-1 bg-blue-100 dark:bg-blue-900/30 rounded-full border border-blue-200 dark:border-blue-700 cursor-pointer hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors relative"
        ...
        >
          ...
          <span class="text-sm font-bold text-blue-900 dark:text-blue-100">{{ onlineUsersCount }}</span>
        </div>
        ...
        <Teleport to="body">
          <div
            ...
            class="fixed w-[200px] bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl p-3"
            :style="dropdownStyle"
            style="z-index: 99999;"
            @mouseenter="handleOnlineUsersMouseEnter"
            @mouseleave="handleOnlineUsersMouseLeave"
          >
            <div class="space-y-2">
              <div class="flex justify-between items-center">
                <span class="text-gray-700 dark:text-gray-300 text-gray-600 dark:text-gray-400">{{ $t('page.header.guestOnline') }}:</span>
                <span class="text-gray-700 dark:text-gray-300 font-medium text-gray-900 dark:text-gray-100">{{ onlineUsersDetails.guestOnline }}</span>
              </div>
              <div class="flex justify-between items-center">
                <span class="text-gray-700 dark:text-gray-300 text-gray-600 dark:text-gray-400">{{ $t('page.header.suspectedBots') }}:</span>
                <span class="text-gray-700 dark:text-gray-300 font-medium text-gray-900 dark:text-gray-100">{{ onlineUsersDetails.suspectedBots }}</span>
              </div>
              <div class="flex justify-between items-center">
                <span class="text-gray-700 dark:text-gray-300 text-gray-600 dark:text-gray-400">{{ $t('page.header.lobbyOnline') }}:</span>
                <span class="text-gray-700 dark:text-gray-300 font-medium text-gray-900 dark:text-gray-100">{{ onlineUsersDetails.lobbyOnline }}</span>
              </div>
              <div class="flex justify-between items-center">
                <span class="text-gray-700 dark:text-gray-300 text-gray-600 dark:text-gray-400">{{ $t('page.header.selfOperatedOnline') }}:</span>
                <span class="text-gray-700 dark:text-gray-300 font-medium text-gray-900 dark:text-gray-100">{{ onlineUsersDetails.selfOperatedOnline }}</span>
              </div>
              <div class="flex justify-between items-center">
                <span class="text-gray-700 dark:text-gray-300 text-gray-600 dark:text-gray-400">{{ $t('page.header.thirdPartyOnline') }}:</span>
                <span class="text-gray-700 dark:text-gray-300 font-medium text-gray-900 dark:text-gray-100">{{ onlineUsersDetails.thirdPartyOnline }}</span>
              </div>
            </div>
          </div>
        </Teleport>
    ...
    <template #notification>
      <div class="flex-center mr-2 h-full">
        <button 
          ...
          class="bell-button text-foreground relative p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors" 
        ...
        >
          ...
          <span
            ...
            class="absolute -right-1 -top-1 bg-red-500 text-white text-xs rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1 font-medium"
          >
        ...
        <Teleport to="body">
          <div
            ...
            class="notification-popup fixed w-[300px] bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-gray-100 rounded-lg shadow-xl"
          ...
          >
            ...
            <div class="text-center py-3 border-b border-gray-200 dark:border-gray-700">
              <div class="text-gray-900 dark:text-gray-100 font-medium text-lg">消息</div>
            </div>
            
            <!-- Notification Categories -->
            <div class="py-2">
              <div class="px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer" @click="handleNotificationClick('onlineRechargeError')">
                🔊 {{ $t('page.header.onlineRechargeError') }}: {{ notificationCategories.onlineRechargeError }}
              </div>
              <div class="px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer" @click="handleNotificationClick('transferPendingReview')">
                🔊 转账待审核: {{ notificationCategories.transferPendingReview }}
              </div>
              <div class="px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer" @click="handleNotificationClick('orderPendingReview')">
                🔊 补单待审核: {{ notificationCategories.orderPendingReview }}
              </div>
              <div class="px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer" @click="handleNotificationClick('withdrawalPendingPayment')">
                🔊 提现待出款: {{ notificationCategories.withdrawalPendingPayment }}
              </div>
              <div class="px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer" @click="handleNotificationClick('gameQuotaLost')">
                🔊 游戏额度丢失: {{ notificationCategories.gameQuotaLost }}
              </div>
              <div class="px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer" @click="handleNotificationClick('riskControlPendingReview')">
                🔊 待风控审核: {{ notificationCategories.riskControlPendingReview }}
              </div>
              <div class="px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer" @click="handleNotificationClick('activityDistributionReview')">
                🔊 活动派发审核: {{ notificationCategories.activityDistributionReview }}
              </div>
              <div class="px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer" @click="handleNotificationClick('activityApplicationReview')">
                🔊 活动申请审核: {{ notificationCategories.activityApplicationReview }}
              </div>
              <div class="px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer" @click="handleNotificationClick('loanApplicationReview')">
                🔊 借款申请审核: {{ notificationCategories.loanApplicationReview }}
              </div>
              <div class="px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer" @click="handleNotificationClick('prizeDistributionMonitoringReview')">
                🔊 派奖监控审核: {{ notificationCategories.prizeDistributionMonitoringReview }}
              </div>
            </div>
          </div>
        </Teleport>
