...
const totalRewards = computed(() => 
  tableData.value.reduce((sum, item) => sum + (item.currentParticipants * item.claimLimit), 0)
);
...
const columns = computed<DataTableColumns<Activity>>(() => [
  ...
  },
  {
    ...
    key: 'category',
    width: 120,
    render: (row) => h(NTag, { type: 'info', size: 'small' }, { default: () => row.category }),
  },
  {
    title: '活动子种类',
    key: 'type',
    width: 140,
    render: (row) => h(NTag, { type: 'default', size: 'small' }, { default: () => row.type }),
  },
  {
    title: '参与会员',
    key: 'memberScope',
  ...
  },
  {
    ...
    render: (row) => h(
      ...
      { size: 'small' },
      {
        default: () => row.platforms.map(platform => 
          h(NTag, { type: 'success', size: 'small' }, { default: () => platform })
        ),
      }
    ),
  },
  ...
  },
  {
    ...
    render: (row) => h(
      ...
      [
        ...
        h('div', { class: 'text-sm text-gray-500' }, `限制：${row.maxParticipants || '无限制'}`),
      ]
    ),
  },
  {
    ...
    render: (row) => h(
      ...
      {},
      [
        h('div', { class: 'text-sm font-medium text-green-600' }, `${(row.currentParticipants * row.claimLimit).toFixed(2)}`),
...
const fetchActivityList = async () => {
  ...
  try {
    ...
    tableData.value = response.list.filter(item => 
      item.status === 'CLOSED' || item.status === 'ENDED'
    );
