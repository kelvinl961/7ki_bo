    ...
    <div class="modal-content-wrapper">
        ...
        <div class="space-y-6">
          ...
          >
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
              <h3 class="text-sm font-medium text-gray-700 mb-4">基础信息</h3>
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
              <h3 class="text-sm font-medium text-gray-700 mb-4">任务时效</h3>
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
              <h3 class="text-sm font-medium text-gray-700 mb-4">领取方式</h3>
            ...
            <!-- Claim Time Section -->
            <div class="bg-gray-50 p-4 rounded-lg">
              <h3 class="text-sm font-medium text-gray-700 mb-4">领取时间</h3>
            ...
            <!-- Claim Entry Section -->
            <div class="bg-gray-50 p-4 rounded-lg">
              <h3 class="text-sm font-medium text-gray-700 mb-4">领取入口</h3>
              <div class="grid grid-cols-2 gap-3">
                <n-checkbox v-model:checked="formData.claimEntries!.androidApp">Android APP可领取</n-checkbox>
                <n-checkbox v-model:checked="formData.claimEntries!.iosApp">IOS APP可领取</n-checkbox>
                <n-checkbox v-model:checked="formData.claimEntries!.h5">马甲包</n-checkbox>
                <n-checkbox v-model:checked="formData.claimEntries!.pwa">PWA体验</n-checkbox>
                <n-checkbox v-model:checked="formData.claimEntries!.iosWeb">IOS触登录</n-checkbox>
                <n-checkbox v-model:checked="formData.claimEntries!.pcWeb">PC可领取</n-checkbox>
                <n-checkbox v-model:checked="formData.claimEntries!.androidH5">Android H5可领取</n-checkbox>
                <n-checkbox v-model:checked="formData.claimEntries!.iosH5">IOS H5可领取</n-checkbox>
              </div>
              
              <!-- Additional Entry Options -->
              <div class="mt-4 space-y-2">
                <n-checkbox v-model:checked="formData.claimEntries!.sameDeviceOnce">同设备只需完成一次</n-checkbox>
                <n-checkbox v-model:checked="formData.claimEntries!.webLoginOnce">PC可领取</n-checkbox>
                <n-checkbox v-model:checked="formData.claimEntries!.deviceBindingOnce">同设备绑定只能领取1次</n-checkbox>
              </div>
            </div>
          </n-form>
        </div>
      ...
      <n-tab-pane name="restrictions" tab="限制条件">
        <div class="space-y-6">
          ...
          <div class="bg-gray-50 p-4 rounded-lg">
            <h3 class="text-sm font-medium text-gray-700 mb-4">更多奖领取限制</h3>
          ...
          <div class="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
            <h3 class="text-sm font-medium text-gray-700 mb-4">稽核倍数设置</h3>
          ...
          <div class="bg-gray-50 p-4 rounded-lg">
            <h3 class="text-sm font-medium text-gray-700 mb-4">登录跳转方式</h3>
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-xs text-gray-600 mb-2">登录前跳转方式</label>
                <n-radio-group v-model:value="formData.loginRedirectMethod!.beforeLogin">
                  <n-space vertical>
              ...
              <div>
                <label class="block text-xs text-gray-600 mb-2">登录后跳转方式</label>
                <n-radio-group v-model:value="formData.loginRedirectMethod!.afterLogin">
                  <n-space vertical>
                    <n-radio value="high_frequency">高频跳转</n-radio>
                    <n-radio value="custom">自定义</n-radio>
                  </n-space>
            ...
            <div class="mt-4">
              <n-checkbox v-model:checked="formData.loginRedirectMethod!.directAfterRecharge">
                充值后直接跳转
              </n-checkbox>
            </div>
          </div>
          ...
          <div class="bg-gray-50 p-4 rounded-lg">
            <h3 class="text-sm font-medium text-gray-700 mb-4">奖励获取规则</h3>
            <div class="space-y-4">
              <div>
                <label class="block text-xs text-gray-600 mb-1">规则说明</label>
                <n-radio-group v-model:value="formData.rewardRules!.explanationType">
                  <n-space vertical>
              ...
              <div v-if="formData.rewardRules!.explanationType === 'custom'">
                <label class="block text-xs text-gray-600 mb-1">自定义规则说明</label>
                <n-input
...
import { createTaskCenter, updateTaskCenter, type TaskCenter, type TaskCenterCreateRequest } from '#/api/taskCenter';
...
const handleSubmit = async () => {
  ...
  try {
    ...
    console.log('🎯 Full formData object being submitted:', JSON.stringify(formData, null, 2));
    ...
    console.log('🔍 Required field check:', {
      ...
      taskConditions_targetValue: formData.taskConditions?.targetValue || 'MISSING',
      taskConditions_conditionType: formData.taskConditions?.conditionType || 'MISSING',
      platformCheck: {
        android: formData.platformAndroid,
        ios: formData.platformIOS, 
        h5: formData.platformH5,
        pc: formData.platformPC,
        atLeastOne: formData.platformAndroid || formData.platformIOS || formData.platformH5 || formData.platformPC
      },
      claimCheck: {
        ...
        h5: formData.claimH5,
        pc: formData.claimPC,
        atLeastOne: formData.claimAndroidApp || formData.claimIOSApp || formData.claimH5 || formData.claimPC
      }
    });
    
    await (props.isEdit 
      ? updateTaskCenter(props.formData?.id || 0, { ...formData, id: props.formData?.id })
