<template>
  <Page>
    <n-card title="代理模式">
      ...
      <n-form ref="filterFormRef" :model="filterForm" label-placement="left" inline size="small" label-width="90">
        <n-form-item label="代理模式名称">
          <n-input v-model:value="filterForm.name" placeholder="请输入模式名称" style="width: 220px" />
        </n-form-item>
        <n-form-item label="币种">
          <n-select v-model:value="filterForm.currency" :options="currencyOptions" placeholder="选择币种" style="width: 140px" />
        </n-form-item>
        <n-form-item label="模式来源">
          <n-select v-model:value="filterForm.source" :options="sourceOptions" placeholder="选择来源" style="width: 140px" />
        </n-form-item>
        <n-form-item label="启用状态">
          <n-select v-model:value="filterForm.isEnabled" :options="enabledOptions" placeholder="选择状态" style="width: 140px" />
        </n-form-item>
        <n-form-item label="是否默认">
          <n-select v-model:value="filterForm.isDefault" :options="defaultOptions" placeholder="选择默认" style="width: 140px" />
        </n-form-item>
        <n-form-item label="结算周期">
          <n-select v-model:value="filterForm.settlementCycle" :options="settlementCycleOptions" placeholder="选择周期" style="width: 140px" />
        </n-form-item>
        <n-form-item>
    ...
    <!-- 新增/编辑 代理模式（按截图进行布局） -->
    <n-modal v-model:show="modalVisible" :title="isEdit ? '编辑代理模式' : '新增代理模式'" :mask-closable="false" preset="card" style="width: 1200px; max-width: 95vw">
      <n-form :model="formData" label-placement="left" label-width="120">
        <n-grid :cols="2" :x-gap="24" :y-gap="16">
          <!-- 左侧表单 -->
          <n-gi>
            <n-form-item label="币种" required>
              <n-select v-model:value="formData.currency" :options="currencyOptions" placeholder="请选择币种" />
            </n-form-item>
            
            <n-form-item label="代理模式名称" required>
              <n-input v-model:value="formData.name" placeholder="请输入代理模式名称" />
            </n-form-item>
            ...
            <n-form-item label="佣金计算层数" required>
              <n-select v-model:value="formData.calcLevels" :options="calcLevelsOptions" placeholder="请选择" />
            </n-form-item>
          ...
          <n-gi>
            <n-form-item label="代理教程设置">
              ...
              <div style="margin-top: 8px;">
                <n-select v-model:value="formData.tutorialTemplate" :options="tutorialOptions" placeholder="请选择模版" />
            ...
            <!-- 富文本编辑器区域 -->
            <div style="margin-top: 16px; border: 1px solid #e5e7eb; border-radius: 6px; min-height: 300px;">
              <div style="border-bottom: 1px solid #e5e7eb; padding: 8px 12px; background-color: #f9fafb;">
                <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                  <!-- 工具栏按钮 -->
                  <n-select v-model:value="formData.fontSize" :options="fontSizeOptions" size="small" style="width: 80px;" />
                  <n-select v-model:value="formData.fontFamily" :options="fontFamilyOptions" size="small" style="width: 100px;" />
              ...
              <div style="border-top: 1px solid #e5e7eb; padding: 8px 12px; text-align: right; font-size: 12px; color: #666;">
                0/255
              </div>
            </div>
        ...
        <n-tabs type="line">
          <n-tab-pane v-for="gt in gameTypes" :key="gt.value" :name="gt.value" :tab="gt.label">
            <div class="flex items-center justify-between mb-2">
              <div class="text-sm text-gray-500">
                当前 {{ gt.label }} 共 {{ tierMap[gt.value]?.length || 0 }} 个层级
              </div>
              <div class="flex items-center">
                <n-button 
                  ...
                  tertiary 
                  type="info"
                  :disabled="!tierMap[gt.value] || tierMap[gt.value].length === 0"
                >
                  复制此配置到全部
                </n-button>
              <n-button size="small" style="margin-left:8px" @click="addTier(gt.value)" type="primary">新增层级</n-button>
              </div>
            </div>
            <n-table size="small" bordered>
              ...
              <tbody>
                <tr v-for="(row, idx) in tierMap[gt.value]" :key="row.key">
                  <td>
                    <n-input-number v-model:value="row.minUsers" :min="0" placeholder="0" style="width:140px" />
                  </td>
                  <td>
                    <n-input-number 
                  ...
                  <td>
                    <n-input-number v-model:value="row.amount" :min="0" :precision="2" placeholder="0.00" style="width:140px" disabled />
                  </td>
                  <td>
                    <n-space>
                      <n-button size="tiny" text type="primary" @click="addTier(gt.value, idx)">新增</n-button>
                      <n-button size="tiny" text type="error" @click="removeTier(gt.value, idx)">删除</n-button>
                    </n-space>
                  </td>
                </tr>
              </tbody>
      ...
      <template #action>
        ...
        <n-button type="primary" :loading="submitLoading" @click="handleConfirm">确认</n-button>
      </template>
    </n-modal>
    ...
    >
      <div class="agent-level-settings">
        <div class="flex justify-between items-center mb-4">
          <div class="flex gap-2">
            <n-button type="success" size="small" @click="handleAutoUpgradeAgents">
              自动升级代理
            </n-button>
          </div>
          <div class="flex gap-2">
            <n-button type="primary" size="small" @click="handleAddAgentLevel">新增</n-button>
            <n-button type="warning" size="small" @click="handleBatchSaveAgentLevels">批量保存</n-button>
          </div>
        </div>
        ...
        <div class="flex justify-center mt-6">
          <n-button type="primary" size="large" @click="agentLevelModalVisible = false">
            关闭
          </n-button>
        </div>
      </div>
    ...
    >
      <div v-if="editingAgentLevel">
        <n-form :model="editingAgentLevel" label-placement="left" label-width="120px">
          ...
          <n-form-item label="描述">
            <n-input v-model:value="editingAgentLevel.description" type="textarea" :rows="3" />
          </n-form-item>
        ...
        <div class="flex justify-end gap-2 mt-4">
          ...
          <n-button 
            type="primary" 
            @click="editingAgentLevel?.id ? handleSaveAgentLevel() : handleCreateAgentLevel()"
          >
    ...
    >
      <div class="net-profit-settings">
        <div class="section-title" style="margin-bottom: 16px; font-weight: bold; color: #333;">
          净盈利排除平台运营成本
        </div>
        ...
        <div class="checkbox-group" style="display: flex; gap: 24px; flex-wrap: wrap;">
          ...
          <n-checkbox v-model:checked="netProfitSettings.excludePreviousBalance">
            上期结余
          </n-checkbox>
        </div>
        
        <div class="flex justify-end gap-2 mt-6">
          <n-button @click="netProfitModalVisible = false" :disabled="netProfitLoading">取消</n-button>
          <n-button type="primary" @click="handleSaveNetProfitSettings" :loading="netProfitLoading">确认</n-button>
        </div>
      </div>
    </n-modal>
    ...
    >
      <div class="agent-public-settings">
        <!-- 默认代理模式设置 -->
        <div class="section">
          <h3 style="margin-bottom: 16px; font-weight: bold;">默认代理模式设置</h3>
          <n-grid :cols="2" :x-gap="16">
            <n-gi>
              <n-form-item label="币种">
                <n-select v-model:value="agentPublicSettings.currency" :options="[
                  { label: 'BRL', value: 'BRL' },
                  { label: 'USD', value: 'USD' }
                ]" />
              </n-form-item>
            ...
            <n-gi>
              <n-form-item label="代理模式">
                <n-select v-model:value="agentPublicSettings.defaultAgentMode" :options="[
        ...
        <div class="section" style="margin-top: 24px;">
          <h3 style="margin-bottom: 16px; font-weight: bold;">返佣前端显示格式设置</h3>
          ...
          <div style="margin-top: 16px;">
            ...
            <n-table size="small" bordered>
              ...
              <tbody>
                <tr v-for="(row, index) in agentPublicSettings.rebateTable" :key="index">
                  <td style="text-align: center;">{{ row.sequence }}</td>
                  <td>
                    <n-input-number v-model:value="row.performance" :min="0" style="width: 100%;" />
                  </td>
                  <td>
                    <n-input-number v-model:value="row.rebateRate" :min="0" :max="100" style="width: 100%;" suffix="%" />
                  </td>
                  <td style="text-align: center;">
                    <n-button size="tiny" type="error" @click="removeRebateRow(index)" :disabled="agentPublicSettings.rebateTable.length === 1">
                      删除
                    </n-button>
                  </td>
                </tr>
        ...
        <div class="section" style="margin-top: 24px;">
          ...
          <n-grid :cols="2" :x-gap="16" style="margin-top: 8px;">
            <n-gi>
              <n-input v-model:value="agentPublicSettings.settlementStartTime" placeholder="07:00:00" style="width: 100%;" />
            </n-gi>
            <n-gi style="display: flex; align-items: center;">
              <span style="margin: 0 8px;">-</span>
              <n-input v-model:value="agentPublicSettings.settlementEndTime" placeholder="23:59:59" style="width: 100%;" />
            </n-gi>
          </n-grid>

          <!-- 佣金积分倍数 -->
          <n-form-item label="佣金积分倍数" style="margin-top: 16px;">
            <n-radio-group v-model:value="agentPublicSettings.commissionMultiplier">
              <n-space>
          ...
          <n-form-item label="有效人数计算" style="margin-top: 16px;">
            <div style="display: flex; gap: 16px; align-items: center;">
              ...
              <n-input-number v-model:value="agentPublicSettings.validMemberCalculation" :min="0" style="width: 120px;" />
            </div>
          </n-form-item>
          ...
          <n-form-item label="" style="margin-top: 8px;">
            <div style="display: flex; gap: 16px; align-items: center;">
              ...
              <n-input-number v-model:value="agentPublicSettings.subordinateValidBet" :min="0" style="width: 120px;" />
            </div>
          </n-form-item>

          <!-- 直属下级单个会员返佣上限 -->
          <n-form-item label="直属下级单个会员返佣上限" style="margin-top: 16px;">
            <div style="display: flex; gap: 16px; align-items: center;">
              <span>日结模式上限</span>
              <n-input-number v-model:value="agentPublicSettings.dailyLimit" :min="0" style="width: 120px;" />
            </div>
          </n-form-item>
          ...
          <n-form-item label="" style="margin-top: 8px;">
            <div style="display: flex; gap: 16px; align-items: center;">
              <span>周结模式上限</span>
              <n-input-number v-model:value="agentPublicSettings.weeklyLimit" :min="0" style="width: 120px;" />
            </div>
          </n-form-item>
          ...
          <n-form-item label="" style="margin-top: 8px;">
            <div style="display: flex; gap: 16px; align-items: center;">
              <span>月结模式上限</span>
              <n-input-number v-model:value="agentPublicSettings.monthlyLimit" :min="0" style="width: 120px;" />
            </div>
          </n-form-item>
        </div>
        
        <div class="flex justify-end gap-2 mt-6">
          <n-button @click="agentPublicModalVisible = false" :disabled="agentPublicLoading">取消</n-button>
          <n-button type="primary" @click="handleSaveAgentPublicSettings" :loading="agentPublicLoading">确认</n-button>
        </div>
      </div>
    </n-modal>
  </Page>
...
import { agentModeApi, gameRebateApi, type GameRebateConfigRequest } from '#/api/agency/agent-mode';
import { 
  getAgentLevelsApi, 
  createAgentLevelApi, 
  updateAgentLevelApi, 
...
const agentPublicSettings = reactive({
  ...
  validMemberCalculation: 0.00,
  subordinateValidBet: 0.00,
  dailyLimit: 0.00,
  weeklyLimit: 0.00,
  monthlyLimit: 0.00,
  rebateTable: [
    { sequence: 1, performance: 1000.00, rebateRate: 3.00 },
    { sequence: 2, performance: 10000.00, rebateRate: 5.00 }
  ]
});
...
const columns: DataTableColumns<AgentModeRow> = [
  ...
  { 
    ...
    render: (row) => {
      return h('span', { 
        style: 'background: #52c41a; color: white; padding: 2px 6px; border-radius: 4px; font-size: 12px;' 
      }, row.currency);
    }
  },
  {
    ...
    width: 150,
    render: (row) => h('div', {}, [
      row.isDefault ? h('span', { style: 'color:#18a058;margin-right:6px;font-weight:bold;' }, '默认') : null, 
      row.name
    ]),
  },
  { 
...
interface TierRow { key: string; minUsers: number; validBet: number; rate: number; amount: number }
...
for (const gt of gameTypes) {
  const initialRow = { key: `${gt.value}-0`, minUsers: 0, validBet: 0, rate: 0.3, amount: 0 };
...
function handleCreate() {
  ...
  for (const gt of gameTypes) {
    const newRow = { key: `${gt.value}-0`, minUsers: 0, validBet: 0, rate: 0.3, amount: 0 };
...
function addTier(gt: string, afterIndex?: number) {
  ...
  const newRow = { key: `${gt}-${Date.now()}-${idx}`, minUsers: 0, validBet: 0, rate: 0, amount: 0 };
  calculateRebateAmount(newRow);
...
function copyConfigToAll(from: string) {
  ...
  for (const gt of gameTypes) {
    if (gt.value === from) continue;
      ...
      message.success(`已成功将"${sourceDisplayName}"的配置复制到 ${copiedCount} 个其他游戏类型`);
    },
    onNegativeClick: () => {
...
async function handleConfirm() {
  try {
    ...
    if (isEdit.value && currentModeId.value) {
      const resp = await agentModeApi.updateAgentMode(currentModeId.value, formData as any);
...
const handleReset = () => {
  Object.assign(filterForm, { name: '', currency: '', source: '', isEnabled: '', isDefault: '', settlementCycle: '' });
  paginationReactive.page = 1;
  loadData();
};
...
// 添加返佣比例行
const addRebateRow = () => {
  const maxSequence = Math.max(...agentPublicSettings.rebateTable.map(r => r.sequence), 0);
...
const loadAgentPublicSettings = async () => {
  try {
    ...
    Object.assign(agentPublicSettings, {
      ...
      rebateTable: data.rebateTable.length > 0 ? data.rebateTable : [
        { sequence: 1, performance: 1000.00, rebateRate: 3.00 },
        { sequence: 2, performance: 10000.00, rebateRate: 5.00 }
      ]
    });
...
const handleBatchSaveAgentLevels = async () => {
  try {
    const updateData: UpdateAgentLevelRequest[] = agentLevels.value.map(level => ({
      id: level.id,
      level: level.level,
      name: level.name,
      promotionCondition: level.promotionCondition,
...
const handleAutoUpgradeAgents = async () => {
  try {
    ...
    if (upgradedCount > 0) {
      ...
      // 显示升级详情
      const upgradeDetails = upgradedAgents.map(agent => 
...
const handleBatchClose = async (selectedRows?: AgentModeRow[]) => {
  const rowsToClose = selectedRows || tableData.value.filter(item => selectedKeys.value.includes(item.id));
  ...
  try {
    ...
    if (response.success && response.data) {
      ...
      Object.keys(groupedConfigs).forEach(gameType => {
        const configs = groupedConfigs[gameType]?.sort((a, b) => (a.tierLevel || 1) - (b.tierLevel || 1)) || [];
      ...
      for (const gt of gameTypes) {
        if (!tierMap[gt.value] || tierMap[gt.value]?.length === 0) {
          const defaultRow = { key: `${gt.value}-${Date.now()}-0`, minUsers: 0, validBet: 0, rate: 0, amount: 0 };
...
const handleStatusToggle = async (row: AgentModeRow, newStatus: boolean) => {
  try {
    ...
    const response = await agentModeApi.updateAgentModeStatus(row.id, newStatus);
