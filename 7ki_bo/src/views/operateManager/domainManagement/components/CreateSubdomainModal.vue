    ...
    >
      ...
      <n-form-item label="选择节点" path="cdnProvider" required>
        <n-space :size="12">
          ...
          >
            <template #icon>
              <n-icon v-if="formData.cdnProvider === cdn.value" :component="CheckmarkCircleOutline" />
            </template>
            <span style="font-size: 16px; font-weight: 500;">{{ cdn.label }}</span>
            <n-tag
              v-if="cdn.tag"
              :type="cdn.tagType"
              size="small"
      ...
      <n-form-item label="作为推广域名">
        <n-space vertical :size="8" style="width: 100%;">
          ...
          <n-alert type="info" :show-icon="false" style="font-size: 13px; padding: 8px 12px;">
            <div style="color: #666;">
              <strong>规则：</strong>必须保持至少一条域名开启该功能。推广域名便于代理推广使用和流量分发。
            </div>
          </n-alert>
        </n-space>
      </n-form-item>
          ...
          <n-radio-group v-model:value="formData.enabledEntrance">
            <n-space vertical>
              ...
              <n-radio value="APP_ONLY">
                <strong>仅极速APP</strong> - 该域名只能通过极速 APP 访问，网页端无法直接访问
              </n-radio>
              <n-radio value="H5_PWA">
                <strong>H5和PWA</strong> - 仅允许 H5 和 PWA 访问，但不会出现在轮巡列表
              </n-radio>
            </n-space>
          </n-radio-group>
          <n-alert type="info" :show-icon="false" style="font-size: 13px; padding: 8px 12px;">
            <div style="color: #666;">
              <strong>支持启用入口的CDN节点：</strong>阿里云、Funnull、Cloudflare (CF)、Yundun、Wangsu-CDN、腾讯云、AWS
            </div>
          </n-alert>
        </n-space>
      </n-form-item>
          ...
          <n-radio-group v-model:value="formData.blockedDevice">
            <n-space vertical>
              ...
              <n-radio value="BLOCK_MOBILE">
                <strong>屏蔽手机端</strong> - PC端可访问，移动端（安卓/iOS）不可访问
              </n-radio>
              <n-radio value="BLOCK_PC">
                <strong>屏蔽PC端</strong> - 移动端（安卓/iOS）可访问，PC端不可访问
              </n-radio>
            </n-space>
          </n-radio-group>
          <n-alert type="info" :show-icon="false" style="font-size: 13px; padding: 8px 12px;">
            <div style="color: #666;">
              <strong>支持屏蔽设备的CDN节点：</strong>阿里云、Funnull、Cloudflare (CF)、Yundun、Wangsu-CDN、腾讯云、AWS
            </div>
          </n-alert>
        </n-space>
      </n-form-item>
      ...
      <n-form-item label="域名配置参考建议：">
        <div style="width: 100%;">
          <div style="margin-bottom: 8px;">
            <a href="#" style="color: #2080f0; text-decoration: none;">详见: 域名管理教程</a>
          </div>
            ...
            <n-scrollbar style="max-height: 300px;">
              <div style="font-size: 13px; line-height: 1.8; color: #666;">
                <p style="margin-bottom: 12px;">
                  <strong>（1）多节点组合：</strong>一定要配置3个或以上节点！因为每个节点都是不同厂商CDN，多个节点组合不仅能自动轮巡和防止封锁，还能独立扛攻击。所以每个功能用途都要配置3个或以上不同CDN厂商，特别是WEB大厅、APP大厅、OSS加速和后端加速域名。
                </p>
                
                <p style="margin-bottom: 12px;">
                  <strong>（2）不是越多越好：</strong>因为域名需要定期检测网络质量、是否被拦截或封锁，所以同个CDN且同个用途只要添加3-10个子域名就够了，并不是越多越好，避免网络检测浪费太多时间。
                </p>
                
                <p style="margin-bottom: 12px;">
                  <strong>（3）如何备用：</strong>只要域名没有启用就不会进行解析，也就不会被外部扫描发现，所以备用的域名可以先添加但不启用，等未来再快速启用。
                </p>
                
                <p style="margin-bottom: 12px;">
                  <strong>（4）主域名说明：</strong>主域名需要先添加到对应的CDN厂家后，且DNS生效后才能创建解析，解析即新增子域名，子域名才是主域名的具体用途，所以添加主域名后一定要到各个功能页面新增子域名才算真正启用。
                </p>
                
                <p style="margin-bottom: 12px;">
                  <strong>（5）子域名说明：</strong>因为主域名被封锁后子域名也会受影响，所以同个主域名且同个功能用途只添加1-2个子域名即可，避免浪费无用功。
                </p>
                
                <p style="margin-bottom: 0;">
                  <strong>（6）大陆市场特别规则：</strong>因大陆的长城防火墙规则特殊，所以建议尽量填写较长且复杂的子域名，而默认的www和@短域名可以作为URL跳转。
                </p>
              </div>
            </n-scrollbar>
          </n-card>
...
const handleConfirm = async () => {
  try {
    ...
    if (hasRootDomain) {
      console.log('⚠️ User included root domain (@) in subdomain creation. Parent domain may already exist.');
    }
    ...
    if (code === 0 && subdomains.length > 0) {
      ...
      if (errors.length > 0) {
        // Partial success
        message.warning(`成功创建 ${subdomains.length} 个子域名，${errors.length} 个失败：${errors.join('; ')}`);
      } else {
        // Full success
      ...
      message.warning(`主域名 ${formData.domainName} 已存在。DNS配置未更新，请在Cloudflare中手动检查。`);
      handleCancel();
