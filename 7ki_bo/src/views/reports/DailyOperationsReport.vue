  ...
  if (timeGranularity.value === 'day') {
    return {
      ...
      '最近3天': (): [number, number] => {
        ...
        const startUTC = convertTimezoneToUTC(threeDaysAgo.getFullYear(), threeDaysAgo.getMonth() + 1, threeDaysAgo.getDate(), 0, 0, 0, tz);
        const endUTC = convertTimezoneToUTC(tzNow.year, tzNow.month, tzNow.day, 23, 59, 59, tz);
        return [startUTC.getTime(), endUTC.getTime()];
      },
      '最近7天': (): [number, number] => {
    ...
    return {
      '上周': (): [number, number] => {
        ...
        const startUTC = convertTimezoneToUTC(lastMonday.getFullYear(), lastMonday.getMonth() + 1, lastMonday.getDate(), 0, 0, 0, tz);
        const endUTC = convertTimezoneToUTC(lastSunday.getFullYear(), lastSunday.getMonth() + 1, lastSunday.getDate(), 23, 59, 59, tz);
        return [startUTC.getTime(), endUTC.getTime()];
      },
      '上上周': (): [number, number] => {
        const now = new Date(tzNow.year, tzNow.month - 1, tzNow.day);
        ...
        const startUTC = convertTimezoneToUTC(twoWeeksAgoMonday.getFullYear(), twoWeeksAgoMonday.getMonth() + 1, twoWeeksAgoMonday.getDate(), 0, 0, 0, tz);
        const endUTC = convertTimezoneToUTC(twoWeeksAgoSunday.getFullYear(), twoWeeksAgoSunday.getMonth() + 1, twoWeeksAgoSunday.getDate(), 23, 59, 59, tz);
        return [startUTC.getTime(), endUTC.getTime()];
      },
      '最近4周': (): [number, number] => {
        const now = new Date(tzNow.year, tzNow.month - 1, tzNow.day);
        ...
        const lastSunday = new Date(now);
        lastSunday.setDate(now.getDate() - daysFromMonday - 1);
        const startUTC = convertTimezoneToUTC(fourWeeksAgoMonday.getFullYear(), fourWeeksAgoMonday.getMonth() + 1, fourWeeksAgoMonday.getDate(), 0, 0, 0, tz);
        const endUTC = convertTimezoneToUTC(lastSunday.getFullYear(), lastSunday.getMonth() + 1, lastSunday.getDate(), 23, 59, 59, tz);
        return [startUTC.getTime(), endUTC.getTime()];
      }
    };
    ...
    return {
      ...
      '最近3个月': (): [number, number] => {
        const threeMonthsAgo = tzNow.month <= 3 ? tzNow.month + 9 : tzNow.month - 3;
        const threeMonthsAgoYear = tzNow.month <= 3 ? tzNow.year - 1 : tzNow.year;
        const startUTC = convertTimezoneToUTC(threeMonthsAgoYear, threeMonthsAgo, 1, 0, 0, 0, tz);
        const endUTC = convertTimezoneToUTC(tzNow.year, tzNow.month, tzNow.day, 23, 59, 59, tz);
        return [startUTC.getTime(), endUTC.getTime()];
      }
    };
...
const renderNumericCell = (row: any, key: string, clickable = false, isCount = false) => {
  ...
  return h('span', { 
    style,
    onClick: (clickable && value !== 0 && !row.isTotal) ? () => handleCellClick(key, row) : undefined
  }, displayValue);
};
...
const columns = computed<DataTableColumns>(() => {
    ...
    children: [
  {
    ...
    width: 100,
    sorter: true,
    render: (row: any) => renderNumericCell(row, 'visitCount', true, true) // isCount
  },
  {
    title: '新增代理',
    key: 'newAgents',
    width: 100,
    sorter: true,
    render: (row: any) => renderNumericCell(row, 'newAgents', false, true) // isCount
  },
  {
    title: '注册',
    key: 'registrations',
    width: 100,
    sorter: true,
    render: (row: any) => renderNumericCell(row, 'registrations', true, true) // isCount
  },
  {
    title: '首充',
    key: 'firstDeposits',
    width: 100,
    sorter: true,
    render: (row: any) => renderNumericCell(row, 'firstDeposits', true, true) // isCount
  },
  {
    title: '登录',
    key: 'logins',
  ...
  },
  {
    ...
    width: 120,
    sorter: true,
    render: (row: any) => renderNumericCell(row, 'totalDepositAmount', true)
  },
  {
    title: '手动充值',
    ...
    width: 120,
    sorter: true,
    render: (row: any) => renderNumericCell(row, 'manualRechargeAmount', true)
  },
  {
    title: '充值人数',
    ...
    width: 100,
    sorter: true,
    render: (row: any) => renderNumericCell(row, 'depositUserCount', true, true) // isCount
  },
  {
    title: '充值次数',
    key: 'depositCount',
    width: 100,
    sorter: true,
    render: (row: any) => renderNumericCell(row, 'depositCount', false, true) // isCount
  },
  {
    title: '提现总额',
    ...
    width: 120,
    sorter: true,
    render: (row: any) => renderNumericCell(row, 'totalWithdrawalAmount', true)
  },
  {
    title: '提现人数',
    ...
    width: 100,
    sorter: true,
    render: (row: any) => renderNumericCell(row, 'withdrawalUserCount', true, true) // isCount
  },
  {
    title: '提现次数',
    key: 'withdrawalCount',
    width: 100,
    sorter: true,
    render: (row: any) => renderNumericCell(row, 'withdrawalCount', false, true) // isCount
  },
  {
    title: '充提差额',
  ...
  },
  {
    ...
    width: 100,
    sorter: true,
    render: (row: any) => renderNumericCell(row, 'bigRPlayers', false, true) // isCount
  }
    ]
  },
    ...
    children: [
  {
    ...
    width: 140,
    sorter: true,
    render: (row: any) => renderNumericCell(row, 'clubDepositSettlementRevenue')
  },
  {
    title: '俱乐部保证金充值',
  ...
  },
  {
    ...
    width: 150,
    sorter: true,
    render: (row: any) => renderNumericCell(row, 'clubDepositTransferOut')
  },
  {
    title: '俱乐部成员上分',
    key: 'clubMemberTopUp',
  ...
  },
  {
    ...
    width: 100,
    sorter: true,
    render: (row: any) => renderNumericCell(row, 'bettorCount', false, true) // isCount
  }
    ]
  },
  ...
  },
  {
    ...
    width: 120,
    sorter: true,
    render: (row: any) => renderNumericCell(row, 'promotionalAmount', true)
  },
  {
    title: '放弃奖励',
  ...
  },
  {
    ...
    width: 120,
    sorter: true,
    render: (row: any) => renderNumericCell(row, 'interestTreasureInterest')
  },
  {
    title: '公积金',
    key: 'providentFund',
  ...
  },
  {
    ...
    width: 100,
    sorter: true,
    render: (row: any) => renderNumericCell(row, 'interestTreasure', true)
  },
  {
    title: '奖励钱包',
    key: 'rewardWallet',
  ...
  const clubKeys = ['clubDepositSettlementRevenue', 'clubDepositRecharge', 'clubDepositTransferOut', 'clubMemberTopUp', 'clubMemberWithdraw', 'clubDeposit'];
  const creditLoanKeys = ['netLending', 'overdueDebt', 'memberLoan', 'memberRepayment'];
  ...
  if (!hasClubData) {
    const clubCategoryIndex = baseColumns.findIndex((col: any) => col.key === 'clubCategory');
    if (clubCategoryIndex !== -1) {
      baseColumns.splice(clubCategoryIndex, 1);
    }
    // Also remove clubDeposit from inventory category
    const inventoryCategory = baseColumns.find((col: any) => col.key === 'inventoryCategory') as any;
    if (inventoryCategory && inventoryCategory.children) {
      const clubDepositIndex = inventoryCategory.children.findIndex((col: any) => col.key === 'clubDeposit');
  ...
  if (!hasCreditLoanData) {
    const creditLoanCategoryIndex = baseColumns.findIndex((col: any) => col.key === 'creditLoanCategory');
    if (creditLoanCategoryIndex !== -1) {
      ...
      const bettorCountCol = creditLoanCategory.children?.find((col: any) => col.key === 'bettorCount');
      if (bettorCountCol) {
        // Move bettorCount to game category
        const gameCategory = baseColumns.find((col: any) => col.key === 'gameCategory') as any;
        if (gameCategory && gameCategory.children) {
          // Remove bettorCount from credit loan
          const bettorCountIndex = creditLoanCategory.children.findIndex((col: any) => col.key === 'bettorCount');
...
const updateDateRangeForGranularity = () => {
  ...
  switch (timeGranularity.value) {
    case 'day':
      ...
      startUTC = convertTimezoneToUTC(tzNow.year, tzNow.month, tzNow.day, 0, 0, 0, tz);
      endUTC = convertTimezoneToUTC(tzNow.year, tzNow.month, tzNow.day, 23, 59, 59, tz);
      break;
    case 'week':
      ...
      startUTC = convertTimezoneToUTC(sevenDaysAgo.getFullYear(), sevenDaysAgo.getMonth() + 1, sevenDaysAgo.getDate(), 0, 0, 0, tz);
      endUTC = convertTimezoneToUTC(today.getFullYear(), today.getMonth() + 1, today.getDate(), 23, 59, 59, tz);
      break;
    case 'month':
...
const fetchData = async () => {
  ...
  try {
    ...
    const startYear = parseInt(startTzParts.find(p => p.type === 'year')!.value);
    const startMonth = parseInt(startTzParts.find(p => p.type === 'month')!.value);
    const startDay = parseInt(startTzParts.find(p => p.type === 'day')!.value);
    
    const endYear = parseInt(endTzParts.find(p => p.type === 'year')!.value);
    const endMonth = parseInt(endTzParts.find(p => p.type === 'month')!.value);
    ...
    const startUTC = convertTimezoneToUTC(startYear, startMonth, startDay, 0, 0, 0, tz);
    const endUTC = convertTimezoneToUTC(endYear, endMonth, endDay, 23, 59, 59, tz);
...
.operations-report-table :deep(.n-data-table-thead > tr:first-child > .n-data-table-th) {
...
.operations-report-table :deep(.n-data-table-thead > tr:first-child > .n-data-table-th .n-data-table-th__title) {
  justify-content: flex-end !important;
}
