...
If the backend doesn't have these endpoints, they need to be created:
...
If the endpoints exist under a different path, update the code to use the correct paths.

**Option 3: Remove `/admin` Prefix**
If endpoints should be `/bet-transactions` instead of `/admin/bet-transactions`, the fallback will handle it automatically.
