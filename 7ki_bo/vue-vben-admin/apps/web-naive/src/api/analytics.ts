...
export async function getTodayDashboardStats(): Promise<DashboardStats> {
  try {
    ...
    let todayWithdraw = 0;
    try {
      const rw = await requestClient.get<{
        todayRecharge: number;
        todayWithdraw: number;
      ...
      console.log('📊 Recharge/Withdraw summary:', {
        todayRecharge,
        todayWithdraw,
      });
    } catch {
      ...
      if (rechargeStats.status === 'fulfilled' && rechargeStats.value?.data) {
        ...
        todayRecharge = dailyStats.reduce(
          (sum, stat) => sum + (stat.actualAmount || 0),
          0,
        );
      }
      if (
        withdrawalStats.status === 'fulfilled' &&
        withdrawalStats.value?.data
      ) {
...
export async function getRankingData(
  type: 'bet' | 'commission' | 'profit' | 'recharge',
  limit = 10,
): Promise<RankingData[]> {
  try {
    // 🌍 Use UTC-3 timezone (America/Sao_Paulo) for date calculations
...
export async function getDashboardAnalytics(): Promise<{
  ...
  try {
    ...
    return {
      ...
      rankings: {
        recharge:
          rechargeRankings.status === 'fulfilled' ? rechargeRankings.value : [],
        bet: betRankings.status === 'fulfilled' ? betRankings.value : [],
        profit:
          profitRankings.status === 'fulfilled' ? profitRankings.value : [],
        commission:
          commissionRankings.status === 'fulfilled'
            ? commissionRankings.value
            : [],
      },
    };
  } catch (error) {
