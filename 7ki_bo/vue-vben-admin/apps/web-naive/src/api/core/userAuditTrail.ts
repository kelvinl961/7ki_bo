...
export interface UserAuditTrail {
  ...
  oldValue?: string;
  newValue?: string;
  result: 'cancelled' | 'failed' | 'pending' | 'success';
  source: 'admin_panel' | 'api' | 'backend' | 'frontend' | 'mobile' | 'system';
  operatorId?: string;
  operatorName?: string;
...
export function getUserAuditTrails(params: AuditTrailFilters) {
  return requestClient.get<AuditTrailListResponse>(
    `/user-audit-trails/users/${params.userId}`,
    {
      params: {
 ...
 * 获取用户审计日志统计
 */
export function getAuditTrailStats(params: {
  endDate?: string;
  startDate?: string;
  userId: number;
}) {
  return requestClient.get<AuditTrailStats>(
    `/user-audit-trails/users/${params.userId}/stats`,
    {
      params: {
        startDate: params.startDate,
        endDate: params.endDate,
...
export function exportUserAuditTrails(params: AuditTrailFilters) {
  return requestClient.get<AuditTrailExportData[]>(
    `/user-audit-trails/users/${params.userId}/export`,
    {
      params: {
        startDate: params.startDate,
        endDate: params.endDate,
 ...
 * 格式化操作时间
 */
export function formatOperationTime(time: Date | string): string {
  try {
...
export function getResultType(
  ...
  const types: Record<string, 'error' | 'info' | 'success' | 'warning'> = {
    success: 'success',
    failed: 'error',
    pending: 'warning',
    cancelled: 'info',
 ...
 * 获取友好的操作描述
 */
export function getFriendlyActionDescription(
  action: string,
  actionDescription?: string,
): string {
