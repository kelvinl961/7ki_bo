...
export interface VIPGlobalSetting {
  ...
  dailyRepeatableClaim: 'repeatable' | 'highest-only';
  ...
  weeklyRepeatableClaim: 'repeatable' | 'highest-only';
  ...
  monthlyRepeatableClaim: 'repeatable' | 'highest-only';
  ...
  birthdayRepeatableClaim: 'repeatable' | 'highest-only';
  ...
  promotionRepeatableClaim: 'repeatable' | 'highest-only';
  
  // ===== 其他设置 =====
  excludedLevels: string[];
  settlementMultiplier: number;
  platformControl: 'unlimited' | 'selected-only' | 'exclude-selected';
  selectedPlatforms: string[];
  rulesType: 'custom' | 'system';
  rulesContent: string;
  
  // ===== 展示设置 =====
  displayMethod: 'list' | 'card';
  iconStyle: 'style1' | 'style2' | 'style3' | 'style4' | 'custom';
  badgeVariant: string;
  
  // ===== 原有字段保留（向后兼容） =====
  // 奖励发放设置
  distributionMethodType: 'DIRECT_CASH' | 'CARD_UNLOCK' | 'BONUS_WALLET' | 'MANUAL_REVIEW';
  ...
  // 规则说明
  ruleType: 'SYSTEM_TEMPLATE' | 'CUSTOM_RULES';
  customRules?: string;
...
export interface VIPReward {
  ...
  vipLevelId: number;
  userId?: number;
  rewardType: 'UPGRADE_BONUS' | 'DAILY_REWARD' | 'WEEKLY_REWARD' | 'MONTHLY_REWARD' | 'BIRTHDAY_REWARD' | 'REBATE' | 'CUSTOM';
  amount: number;
  currency: string;
  description?: string;
  status: 'PENDING' | 'DISTRIBUTED' | 'FAILED' | 'CANCELLED' | 'EXPIRED';
  scheduledAt?: string;
...
export interface VIPUpgradeHistory {
  ...
  fromLevelId?: number;
  toLevelId: number;
  upgradeType: 'AUTOMATIC' | 'MANUAL' | 'SYSTEM' | 'ADMIN';
...
export interface VIPLevelListParams {
  ...
  level?: number;
  currency?: string;
  sortBy?: 'level' | 'name' | 'currentMemberCount' | 'createdAt';
  sortOrder?: 'asc' | 'desc';
}
...
export interface VIPRewardListParams {
  ...
  dateFrom?: string;
  dateTo?: string;
  sortBy?: 'createdAt' | 'amount' | 'scheduledAt' | 'distributedAt';
  sortOrder?: 'asc' | 'desc';
}
...
export interface VIPUpgradeHistoryParams {
  ...
  dateFrom?: string;
  dateTo?: string;
  sortBy?: 'createdAt' | 'bonusAmount' | 'depositAmount' | 'betAmount';
  sortOrder?: 'asc' | 'desc';
}
...
export const getVIPLevels = async (params: VIPLevelListParams = {}): Promise<PaginatedResponse<VIPLevel>> => {
  console.log('🔄 getVIPLevels API call started with params:', params);
  try {
    // Use the VIP levels endpoint
    const response = await requestClient.get<PaginatedResponse<VIPLevel>>('/vip/levels', { params });
    ...
    if (response && (response as any).success === true && (response as any).data) {
    ...
    } else if (response && (response as any).list && (response as any).pagination) {
...
export const getVIPLevelById = async (id: number): Promise<VIPLevel> => {
  const response = await requestClient.get<ApiResponse<VIPLevel>>(`/vip/levels/${id}`);
  return (response as any).data || response; // Handle response format
};

export const createVIPLevel = async (data: CreateVIPLevelData): Promise<VIPLevel> => {
  const response = await requestClient.post<ApiResponse<VIPLevel>>('/vip/levels', data);
  return (response as any).data || response; // Handle response format
};

export const updateVIPLevel = async (id: number, data: Partial<CreateVIPLevelData>): Promise<VIPLevel> => {
  const response = await requestClient.put<ApiResponse<VIPLevel>>(`/vip/levels/${id}`, data);
  return (response as any).data || response; // Handle response format
};

export const bulkUpdateVIPLevels = async (levels: any[]): Promise<VIPLevel[]> => {
  const response = await requestClient.put<ApiResponse<VIPLevel[]>>('/vip/levels/bulk-update', { levels });
  return (response as any).data || response; // Handle response format
};
...
export const toggleVIPLevelStatus = async (id: number, isActive: boolean): Promise<VIPLevel> => {
  const response = await requestClient.post<ApiResponse<VIPLevel>>('/vip/levels/toggle-status', { id, isActive });
  return (response as any).data || response; // Handle response format
};

export const getVIPLevelOptions = async (): Promise<Array<{ id: number; level: number; name: string; color: string }>> => {
  const response = await requestClient.get<ApiResponse<Array<{ id: number; level: number; name: string; color: string }>>>('/vip/levels/options');
  return (response as any).data || response; // Handle response format
};
...
export const getVIPGlobalSettings = async (): Promise<VIPGlobalSetting> => {
  ...
  try {
    const response = await requestClient.get<ApiResponse<VIPGlobalSetting>>('/vip/settings');
...
export const updateVIPGlobalSettings = async (data: Partial<VIPGlobalSetting>): Promise<VIPGlobalSetting> => {
  const response = await requestClient.put<ApiResponse<VIPGlobalSetting>>('/vip/settings', data);
  return (response as any).data || response; // Handle response format
};
...
export const getVIPRewards = async (params: VIPRewardListParams = {}): Promise<PaginatedResponse<VIPReward>> => {
  const response = await requestClient.get<ApiResponse<PaginatedResponse<VIPReward>>>('/vip/rewards', { params });
  return (response as any).data || response; // Handle response format
};

export const createVIPReward = async (data: CreateVIPRewardData): Promise<VIPReward> => {
  const response = await requestClient.post<ApiResponse<VIPReward>>('/vip/rewards', data);
  return (response as any).data || response; // Handle response format
};

export const updateVIPReward = async (id: number, data: Partial<CreateVIPRewardData>): Promise<VIPReward> => {
  const response = await requestClient.put<ApiResponse<VIPReward>>(`/vip/rewards/${id}`, data);
  return (response as any).data || response; // Handle response format
};

export const bulkDistributeRewards = async (rewardIds: number[], distributionMethod?: string): Promise<VIPReward[]> => {
  const response = await requestClient.post<ApiResponse<VIPReward[]>>('/vip/rewards/bulk-distribute', { 
    rewardIds, 
    distributionMethod 
  });
  return (response as any).data || response; // Handle response format
...
export const getVIPUpgradeHistory = async (params: VIPUpgradeHistoryParams = {}): Promise<PaginatedResponse<VIPUpgradeHistory>> => {
  const response = await requestClient.get<ApiResponse<PaginatedResponse<VIPUpgradeHistory>>>('/vip/upgrades', { params });
  return (response as any).data || response; // Handle response format
};

export const createVIPUpgrade = async (data: CreateVIPUpgradeData): Promise<VIPUpgradeHistory> => {
  const response = await requestClient.post<ApiResponse<VIPUpgradeHistory>>('/vip/upgrades', data);
  return (response as any).data || response; // Handle response format
};
...
export const getVIPStatistics = async (params: VIPStatisticsParams = {}): Promise<{ statistics: VIPStatistics[]; summary: any }> => {
  const response = await requestClient.get<ApiResponse<{ statistics: VIPStatistics[]; summary: any }>>('/vip/statistics', { params });
  return (response as any).data || response; // Handle response format
};

export const getVIPDashboard = async (): Promise<VIPDashboard> => {
  const response = await requestClient.get<ApiResponse<VIPDashboard>>('/vip/dashboard');
  return (response as any).data || response; // Handle response format
};
...
export const evaluateVIPLevelsApi = async (userId?: number): Promise<any> => {
  ...
  const response = await requestClient.post<ApiResponse<any>>('/vip/evaluate', payload);
