  ...
  <div class="user-audit-trail-tab">
    ...
    <n-card title="查询条件" class="mb-4 query-card">
      ...
      <div class="flex gap-2 flex-wrap">
        <n-button type="primary" @click="loadAuditTrails" :loading="loading" class="action-button">
          查询日志
        </n-button>
        <n-button @click="handleReset" class="action-button">
          重置
        </n-button>
        <n-button @click="loadAuditTrails" :disabled="loading" class="action-button">
          刷新
        </n-button>
        <n-button @click="handleExport" :loading="exportLoading" class="action-button">
          导出Excel
        </n-button>
      </div>
      
      <!-- Query Status -->
      <div v-if="lastQueryInfo" class="mt-4 pt-3 border-t border-gray-200">
      ...
      <template #header-extra>
        <div class="flex items-center gap-3">
          <div class="flex items-center gap-2 text-sm text-gray-600">
            <span class="font-medium">共 {{ pagination.itemCount }} 条记录</span>
            <span class="text-gray-400">|</span>
            <span>第 {{ pagination.page }} / {{ Math.ceil(pagination.itemCount / pagination.pageSize) }} 页</span>
          </div>
        </div>
      </template>
    ...
    <!-- Detail Modal -->
    <n-modal v-model:show="showDetailModal" preset="card" title="操作日志详情" style="width: 800px">
      <div v-if="selectedAuditTrail" class="space-y-4">
        <n-descriptions bordered :column="2" size="small">
          ...
          <n-descriptions-item label="浏览器">
            {{ selectedAuditTrail.browserName || '-' }} {{ selectedAuditTrail.browserVersion || '' }}
          </n-descriptions-item>
          <n-descriptions-item label="操作系统">
            {{ selectedAuditTrail.operatingSystem || '-' }} {{ selectedAuditTrail.osVersion || '' }}
          </n-descriptions-item>
          <n-descriptions-item label="设备信息">
            {{ selectedAuditTrail.deviceBrand || '-' }} {{ selectedAuditTrail.deviceModel || '' }}
          </n-descriptions-item>
        </n-descriptions>
        ...
        <div v-if="selectedAuditTrail.oldValue || selectedAuditTrail.newValue" class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div v-if="selectedAuditTrail.oldValue">
            <div class="text-sm font-medium text-gray-700 mb-2">变更前</div>
            <div class="text-sm text-gray-600 p-2 bg-red-50 rounded border border-red-200">
              {{ selectedAuditTrail.oldValue }}
            </div>
          </div>
          <div v-if="selectedAuditTrail.newValue">
            <div class="text-sm font-medium text-gray-700 mb-2">变更后</div>
            <div class="text-sm text-gray-600 p-2 bg-green-50 rounded border border-green-200">
        ...
        <div v-if="selectedAuditTrail.errorMessage" class="space-y-2">
          ...
          <div class="text-sm text-red-600 p-2 bg-red-50 rounded border border-red-200">
...
import { NCard, NButton, NDatePicker, NDataTable, NSpin, NTag, NSelect, NModal, NDescriptions, NDescriptionsItem, NDivider } from 'naive-ui';
import { 
  getUserAuditTrails, 
  getAuditTrailStats,
...
const auditTrailColumns = [
  {
    ...
    render: (row: UserAuditTrail) => {
      return h('div', { class: 'text-sm' }, formatOperationTime(row.operationTime));
    }
  },
  {
    title: '操作项目',
...
const loadAuditTrails = async () => {
  ...
  try {
    ...
    if (operatorTypeFilter.value) params.operatorType = operatorTypeFilter.value;
...
const handleExport = async () => {
  ...
  try {
    ...
    downloadCSV(csvContent, `用户${props.userId}_操作日志_${new Date().toISOString().split('T')[0]}.csv`);
