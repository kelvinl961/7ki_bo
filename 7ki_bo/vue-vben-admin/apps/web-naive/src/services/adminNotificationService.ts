...
export interface AdminNotification {
  id?: number;
  type: 'deposit_created' | 'deposit_force_processed' | 'deposit_force_cancelled' | 'withdrawal_created' | 'system_alert';
  title: string;
  content: string;
  orderId?: string;
...
export class AdminNotificationService {
   ...
   */
  private static async playNotificationSound(priority: string = 'medium'): Promise<void> {
    ...
    try {
      ...
      this.notificationAudio.volume = volumeLevels[priority as keyof typeof volumeLevels] || 0.7;
  ...
  static async checkPendingWithdrawals(): Promise<void> {
    console.log('🔍 [AdminNotificationService] checkPendingWithdrawals() called');
    console.log('🔍 [AdminNotificationService] requestClient available:', typeof requestClient, requestClient);
    
    try {
      // ✅ Safety check: Ensure requestClient is available
      if (!requestClient || typeof requestClient.get !== 'function') {
        console.error('❌ [AdminNotificationService] requestClient is not available - skipping');
        return;
      }
      ...
      console.log('📡 [AdminNotificationService] Fetching /notifications/counts...');
      const response = await requestClient.get('/notifications/counts');
      console.log('📦 [AdminNotificationService] Got notification counts response:', response);
      
      // ✅ Response interceptor unwraps data, so response IS the data object
      if (response) {
        const pendingCount = (response.withdrawalPendingPayment || 0) + 
                           (response.riskControlPendingReview || 0);
        
        console.log(`📊 [AdminNotificationService] Pending count: ${pendingCount}`);
        
        if (pendingCount > 0) {
          console.log(`🔔 Found ${pendingCount} pending withdrawals on login (old + new)`);
          console.log(`   - 财务待出款: ${response.withdrawalPendingPayment || 0}`);
          console.log(`   - 风控待审核: ${response.riskControlPendingReview || 0}`);
          
          // Play notification sound for ANY pending withdrawal
          console.log('🎵 [AdminNotificationService] Playing notification sound...');
          await this.playNotificationSound('medium');
          console.log('✅ [AdminNotificationService] Sound played successfully');
        } else {
      ...
      console.error('❌ [AdminNotificationService] Failed to check pending withdrawals:');
  ...
  static async initialize(): Promise<void> {
    ...
    console.log('🚀🚀🚀 [AdminNotificationService] initialize() CALLED - START');
    console.log('='.repeat(80));
    
    // Restore sound preference
    this.soundEnabled = this.isSoundEnabled();
    console.log(`🔊 [AdminNotificationService] Sound enabled: ${this.soundEnabled}`);
    ...
    if (typeof window !== 'undefined') {
      ...
      console.log('⏰ [AdminNotificationService] Setting timeout to check withdrawals in 3 seconds...');
      setTimeout(() => {
        console.log('⏰ [AdminNotificationService] Timeout triggered, calling checkPendingWithdrawals...');
        this.checkPendingWithdrawals().catch(err => {
          console.error('❌ [AdminNotificationService] checkPendingWithdrawals failed:', err);
        });
