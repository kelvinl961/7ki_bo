...
export function formatAccountNumber(accountNumber: string): string {
  ...
  const cleaned = accountNumber.replaceAll(/\D/g, '');

  if (cleaned.length <= 4) return accountNumber;

  const start = cleaned.slice(0, 2);
  const end = cleaned.slice(Math.max(0, cleaned.length - 2));
...
export function formatOrderStatus(status: string): {
  ...
  const statusMap: Record<
    string,
    { color: string; text: string; type: string }
  > = {
  ...
  return (
    statusMap[status] || { text: status, color: '#d9d9d9', type: 'default' }
  );
}
