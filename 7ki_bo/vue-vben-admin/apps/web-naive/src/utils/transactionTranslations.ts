...
async function getActivityName(
  ...
  try {
    ...
    console.warn(
      `[translateSubcategory] Failed to fetch activity ${id}:`,
      error,
    );
    return null;
  }
...
export async function preloadActivityNames(transactions: any[]): Promise<void> {
  ...
  transactions.forEach((tx) => {
    if (
      ...
      const id =
        typeof tx.metadata.activityId === 'string'
          ? Number.parseInt(tx.metadata.activityId, 10)
          : tx.metadata.activityId;
      if (!isNaN(id)) {
        activityIds.add(id);
      }
  ...
  const uncachedIds = [...activityIds].filter((id) => {
    const cached = activityNameCache.get(id);
    const expiry = activityNameCacheExpiry.get(id);
  ...
  console.log(
    `[preloadActivityNames] Pre-loading ${uncachedIds.length} activity names...`,
  );
  await Promise.allSettled(uncachedIds.map((id) => getActivityName(id)));
  console.log(
    `[preloadActivityNames] Pre-loaded ${uncachedIds.length} activity names`,
  );
}
...
export function translateSubcategory(
  ...
  if (
    trimmed === 'recharge' && // Check if this is a recharge activity reward (has activityId and activityType)
    metadata?.activityId &&
    metadata?.activityType === 'recharge'
  ) {
    // ✅ PRIORITY 1: Use activity title from metadata if available and valid
    const activityTitle =
      metadata?.activityTitle || metadata?.subcategoryDetails;

    if (
      activityTitle &&
      activityTitle !== 'recharge' &&
      activityTitle.trim() !== ''
    ) {
    ...
    const isFirstDepositFromDescription =
      description.includes('首充奖励') || description.includes('首充');
    const isAccumulateFromDescription =
      description.includes('累计充值奖励') || description.includes('累计充值');

    if (condition === 'first_deposit' || isFirstDepositFromDescription) {
      return '首充活动'; // First Deposit Activity (fallback)
    } else if (
      condition === 'accumulate_recharge' ||
      isAccumulateFromDescription
    ) {
  ...
  if (
    trimmed === 'VIP_UPGRADE_BONUS' ||
    trimmed.toUpperCase() === 'VIP_UPGRADE_BONUS'
  ) {
    let vipLevel = '';
  ...
  if (
    ...
    (trimmed.includes('REWARD') || trimmed.includes('BONUS'))
  ) {
    let vipLevel = '';
    if (metadata?.vipLevel) {
      vipLevel = metadata.vipLevel;
  ...
  if (
    ...
    if (activityId) {
      try {
        ...
        console.warn(
          `[translateSubcategoryAsync] Failed to fetch activity name for ${activityId}:`,
          error,
        );
      }
    }
 ...
 */
export function getTransactionTypeTagType(
  type: string,
