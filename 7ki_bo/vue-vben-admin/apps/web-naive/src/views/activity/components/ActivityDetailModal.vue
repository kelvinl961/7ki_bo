    ...
    <div v-if="mappedActivity" class="activity-detail">
      ...
      <div class="mb-6">
        <div class="flex items-center gap-4 mb-4">
          <div v-if="mappedActivity.imageUrl" class="w-20 h-20 bg-white rounded-lg shadow-md flex items-center justify-center">
            <n-image
          ...
          <div class="flex-1">
            <h2 class="text-2xl font-bold text-gray-800 mb-2">{{ mappedActivity.title }}</h2>
            <div class="flex items-center gap-2">
              <n-tag type="info" size="medium">{{ getCategoryText(mappedActivity.category) }}</n-tag>
              <n-tag v-if="mappedActivity.currencyScope" type="success" size="medium">{{ mappedActivity.currencyScope }}</n-tag>
      ...
      <n-tabs type="line" animated>
        <n-tab-pane name="info" tab="基本信息">
          ...
          >
            ...
            <n-descriptions-item label="赠送金额">
              {{ mappedActivity.claimLimit }} {{ mappedActivity.currencyScope || '' }}
            </n-descriptions-item>
            ...
            <n-descriptions-item label="参与进度">
              <div class="flex items-center gap-2">
                <n-progress
                  ...
                  style="width: 100px"
                />
                <span class="text-sm text-gray-600">{{ mappedActivity.progress }}%</span>
              </div>
            </n-descriptions-item>
            ...
            <n-descriptions-item label="支持平台">
              <n-space v-if="mappedActivity.platforms && mappedActivity.platforms.length > 0" size="small">
                <n-tag
          ...
          >
            ...
            <n-descriptions-item label="活动时长">
              <div class="flex items-center gap-2">
                ...
                <span>{{ getDurationText(mappedActivity.startsAt, mappedActivity.endsAt) }}</span>
              </div>
            </n-descriptions-item>
            ...
            <n-descriptions-item label="剩余时间">
              <div class="flex items-center gap-2">
                ...
                <span :class="getRemainingTimeClass(mappedActivity.timeRemaining)">
            ...
            <n-descriptions-item label="活动状态">
              <div class="flex items-center gap-2">
                <n-icon :color="mappedActivity.isCurrentlyActive ? '#52c41a' : '#d9d9d9'">
                  <CheckmarkCircleOutline />
                </n-icon>
                <span>{{ mappedActivity.isCurrentlyActive ? '进行中' : '未开始/已结束' }}</span>
              </div>
            </n-descriptions-item>
          </n-descriptions>
        </n-tab-pane>
        ...
        <n-tab-pane name="content" tab="内容信息">
          <div class="space-y-4">
            <div v-if="mappedActivity.requirement">
              <h4 class="font-semibold text-gray-800 mb-2">活动要求</h4>
            ...
            <div v-if="mappedActivity.description">
              <h4 class="font-semibold text-gray-800 mb-2">活动描述</h4>
            ...
            <div v-if="mappedActivity.rules">
              <h4 class="font-semibold text-gray-800 mb-2">活动规则</h4>
              <div class="bg-yellow-50 p-3 rounded-lg text-sm text-gray-700 whitespace-pre-line">
                {{ mappedActivity.rules }}
              </div>
            </div>
            
            <div v-if="!mappedActivity.requirement && !mappedActivity.description && !mappedActivity.rules" class="text-center py-8">
        ...
        <n-tab-pane name="stats" tab="统计信息">
          <div class="grid grid-cols-2 gap-4">
            <n-card size="small">
              <n-statistic label="参与人数" :value="mappedActivity.currentParticipants">
                <template #suffix>
                  <span class="text-sm text-gray-500">/ {{ mappedActivity.maxParticipants || '无限制' }}</span>
                </template>
              </n-statistic>
            </n-card>
            ...
            <n-card size="small">
              <n-statistic label="预计支出" :value="(mappedActivity.currentParticipants || 0) * (mappedActivity.claimLimit || 0)">
                <template #suffix>
                  <span class="text-sm text-gray-500">{{ mappedActivity.currencyScope || '' }}</span>
                </template>
              </n-statistic>
            </n-card>
            
            <n-card size="small">
              <n-statistic label="活动天数" :value="mappedActivity.durationDays">
                <template #suffix>
          ...
          <div class="mt-6">
            <h4 class="font-semibold text-gray-800 mb-3">活动时间线</h4>
            <n-timeline>
              <n-timeline-item
                type="success"
              ...
              />
              <n-timeline-item
                :type="new Date(mappedActivity.startsAt) <= new Date() ? 'success' : 'info'"
                :title="formatDateTime(mappedActivity.startsAt)"
                content="活动开始"
              />
              <n-timeline-item
                :type="new Date(mappedActivity.endsAt) <= new Date() ? 'error' : 'warning'"
...
const mappedActivity = computed(() => {
  ...
  return {
    ...
    startsAt: activity.startsAt || activity.startAt || config.startAt || config.startsAt || activity.createdAt,
    endsAt: activity.endsAt || activity.endAt || config.endAt || config.endsAt || activity.updatedAt,
    ...
    currentParticipants: activity.currentParticipants || config.currentParticipants || 0,
    progress: activity.progress || config.progress || 0,
    isActive: activity.isActive !== undefined ? activity.isActive : (config.isActive !== undefined ? config.isActive : true),
    isCurrentlyActive: activity.isCurrentlyActive !== undefined 
    ...
    lastModifiedBy: activity.lastModifiedBy || activity.createdBy || config.lastModifiedBy || '系统',
  };
});
...
const getDurationText = (startAt: string | Date | null | undefined, endAt: string | Date | null | undefined) => {
  if (!startAt || !endAt) {
    return '未设置';
  }
