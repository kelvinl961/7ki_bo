      ...
      <n-card :bordered="false" class="rounded-16px shadow-sm">
        <div class="flex justify-between items-center mb-4">
          <div>
            ...
            <p class="text-sm text-gray-600 mt-1">回调异常订单(含重新代付)</p>
          </div>
          ...
          <div class="flex justify-between items-center">
            <div class="flex items-center gap-4">
              ...
              <div class="flex gap-6 text-sm">
                <span>异常订单总数: <span class="font-semibold text-red-600">{{ paginationReactive.total }}</span></span>
                <span>待重新代付: <span class="font-semibold text-orange-600">{{ statistics.pendingRepayment }}</span></span>
                <span>重新代付成功: <span class="font-semibold text-green-600">{{ statistics.repaymentSuccess }}</span></span>
              </div>
    ...
    >
      <div class="space-y-4">
        ...
        <div class="max-h-60 overflow-y-auto">
          <div v-for="item in rePaymentModal.items" :key="item.id" class="border rounded p-2 mb-2">
            <div class="text-sm">
              ...
              <div><strong>异常原因:</strong> {{ item.callbackError || '回调异常' }}</div>
            </div>
          </div>
        </div>
        <div>
      ...
      <div v-if="detailModal.data" class="space-y-6">
          ...
          <n-card title="异常信息" size="small">
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                ...
                <n-tag :type="getCallbackStatusType(detailModal.data.callbackStatus)">
                  {{ detailModal.data.callbackStatus || '异常' }}
                </n-tag>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">异常原因:</span>
                <span class="text-red-600">{{ detailModal.data.callbackError || '回调超时' }}</span>
              </div>
              <div class="flex justify-between">
              ...
              <div class="flex justify-between">
                ...
                <span>{{ formatDateTime(detailModal.data.lastRetryTime) || '-' }}</span>
              </div>
            </div>
          </n-card>
        </div>
        ...
        <n-card title="回调日志" size="small">
          <div class="max-h-40 overflow-y-auto">
            <div v-for="(log, index) in detailModal.data.callbackLogs" :key="index" class="border-b pb-2 mb-2 last:border-b-0">
              <div class="text-xs text-gray-500">{{ formatDateTime(log.timestamp) }}</div>
              <div class="text-sm">{{ log.message }}</div>
            </div>
            <div v-if="!detailModal.data.callbackLogs?.length" class="text-center text-gray-500 py-4">
              暂无回调日志
            </div>
          </div>
        </n-card>
...
import { ref, reactive, onMounted, computed, h, onUnmounted, watch, defineAsyncComponent } from 'vue';
...
import { ReloadOutline, SearchOutline, RepeatOutline, WarningOutline, DownloadOutline } from '@vicons/ionicons5';
...
const columns: DataTableColumns<RePaymentRecord> = [
  ...
  },
  {
    ...
    render: (row) => h('div', [
      ...
      h('div', { class: 'text-xs text-gray-500' }, row.memberName || '会员层级')
    ])
  },
  {
    title: '申请时间 (操作时间)',
  ...
  },
  {
    ...
    render: (row) => h('div', [
      h('div', { class: 'flex items-center gap-1' }, [
        ...
        h('span', { class: 'bg-red-500 text-white text-xs px-1 rounded' }, '异常')
      ]),
      h('div', { class: 'text-xs text-gray-500' }, 
  ...
  },
  {
    ...
    key: 'backendNotes',
    width: 100,
    render: (row) => h('div', { class: 'text-center text-red-500' }, row.callbackError || '待重新代付')
  },
  {
    title: '三方代付 (代付次数)',
    key: 'thirdParty',
    ...
    render: (row) => {
      ...
      return h('div', { class: 'text-center' }, [
        h('div', displayName),
        h('div', { class: 'text-xs text-gray-500' }, `(${row.retryCount || 0}次)`)
      ]);
    }
  },
  {
...
const fetchData = async () => {
  ...
  try {
    ...
    try {
      const response = await rePaymentApi.getCallbackExceptionWithdrawals(params);
...
const handleRePayment = async () => {
  try {
    ...
    if (rePaymentModal.items.length === 1) {
      // Single re-payment
      try {
        const response = await rePaymentApi.initiateRePayment(rePaymentModal.items[0].id, {
      ...
      try {
        ...
        console.warn('API call failed:', apiError);
        message.success(`成功重新代付 ${rePaymentModal.items.length} 个订单 (模拟)`);
        rePaymentModal.show = false;
        selectedIds.value = [];
        fetchData();
      }
...
const canRePayment = (status: string) => {
  return ['callback_failed', 'callback_timeout', 'repayment_failed'].includes(status);
};
