  ...
  <div class="recharge-order-list">
    ...
    <n-tabs v-model:value="activeTab" type="line" class="mb-4">
        ...
        <n-card class="mb-4">
          <div class="filter-section">
            ...
            <div class="filter-row">
              ...
              <!-- 日期范围选择器 -->
              <div class="flex flex-col">
                <label class="mb-2 text-sm font-medium">开始日期 - 结束日期</label>
                <TimezoneDatePicker
          ...
          <div class="totals-summary">
            <n-space size="large">
              <div class="total-item">
                ...
                <span class="total-value">{{ formatCurrency(totals.rechargeAmount) }}</span>
              </div>
              <div class="total-item">
                <span class="total-label">订单金额合计:</span>
                <span class="total-value">{{ formatCurrency(totals.orderAmount) }}</span>
              </div>
              <div class="total-item">
                <span class="total-label">赠送金额合计:</span>
                <span class="total-value bonus">{{ formatCurrency(totals.bonusAmount) }}</span>
              </div>
              <div class="total-item">
                <span class="total-label">手续费合计:</span>
                <span class="total-value fee">{{ formatCurrency(totals.fee) }}</span>
              </div>
              <div class="total-item">
                <span class="total-label">总上分金额:</span>
                <span class="total-value credited">{{ formatCurrency(totals.totalCredited) }}</span>
              </div>
            </n-space>
          </div>
        </n-card>
        ...
        <n-card class="mb-4">
          <div class="filter-section">
            ...
            <div class="filter-row">
              ...
              />
              <n-button type="primary" @click="handleCategorySearch">搜索</n-button>
              <n-button @click="handleCategoryReset">重置</n-button>
              <div class="ml-auto">
                <n-button type="primary" @click="handleAddCategory">新增充值大类</n-button>
        </div>
            </div>
          </div>
        </n-card>
            ...
            <n-card :bordered="false" class="rounded-16px shadow-sm">
              <div class="flex justify-between items-center">
                ...
                <div class="flex items-center gap-4">
                  <!-- 选择控制 -->
                  <div class="flex gap-2">
                    <n-button size="small" @click="handleCategoryReset">重置筛选</n-button>
                    <n-button size="small" @click="fetchCategoryData">刷新数据</n-button>
                  </div>
                </div>
              </div>
            </n-card>
      ...
      <n-tab-pane name="disabled-channels" tab="已停用通道">
        ...
        <n-card class="mb-4">
          <div class="filter-section">
            ...
            <div class="filter-row">
              ...
              <n-button type="primary" @click="handleDisabledChannelSearch">搜索</n-button>
            ...
            <div class="filter-row">
              <n-space>
                <n-button type="success" @click="handleBatchEnableChannels" :disabled="!selectedDisabledChannels.length">
                  <template #icon>
                    <n-icon><CheckCircle /></n-icon>
                  </template>
                  批量启用 ({{ selectedDisabledChannels.length }})
                </n-button>
                <n-button type="error" @click="handleBatchDeleteChannels" :disabled="!selectedDisabledChannels.length">
                  <template #icon>
                    <n-icon><Trash /></n-icon>
                  </template>
              ...
              <div class="ml-auto">
                <n-tag type="warning" size="medium">
                  ...
                  已停用通道: {{ disabledChannelPaginationConfig.itemCount || 0 }}
                </n-tag>
              </div>
            </div>
          </div>
        ...
        <n-card>
          <template #header>
            <div class="flex items-center justify-between">
              <span class="text-lg font-medium text-gray-700">已停用通道列表</span>
              <n-tooltip trigger="hover">
                <template #trigger>
                  <n-icon size="16" class="text-gray-400 cursor-help">
                    <Information />
                  </n-icon>
                </template>
          ...
          <div v-if="groupedDisabledChannels.length > 0" class="space-y-6">
            <div v-for="group in groupedDisabledChannels" :key="group.category" class="category-group">
              <!-- Category Header -->
                ...
                <div class="flex items-center gap-4 mb-4">
                  <div class="category-icon">
                    ...
                    <div v-else class="w-12 h-12 rounded-lg bg-gray-200 flex items-center justify-center">
                      <span class="text-gray-500 text-xs">{{ group.category }}</span>
                    </div>
                  </div>
                  <div class="flex-1">
                    <h3 class="text-lg font-medium text-gray-800">{{ group.category }}</h3>
                    <p class="text-sm text-gray-500">已停用通道: {{ group.channels.length }} 个</p>
                  </div>
                  <div class="flex items-center gap-2">
                    <n-button 
                      type="success" 
                      size="small"
                      :disabled="!getSelectedChannelsInGroup(group.category).length"
                      @click="handleBatchEnableChannelsInGroup(group.category)"
                    >
                      <template #icon>
                        <n-icon><CheckCircle /></n-icon>
                      </template>
                      批量启用 ({{ getSelectedChannelsInGroup(group.category).length }})
                    </n-button>
                    <n-button 
                      v-if="!group.expanded"
                      type="info" 
          ...
          <!-- Empty State -->
          <div v-else-if="!disabledChannelTableLoading" class="text-center py-8">
            <n-empty description="暂无已停用的通道">
              <template #extra>
                <n-button type="primary" @click="activeTab = 'third-party-channels'">
                  查看活跃通道
                </n-button>
              </template>
            </n-empty>
          ...
          <!-- Overall Pagination -->
          <div v-if="groupedDisabledChannels.length > 0" class="flex justify-end mt-4">
            <n-pagination
        ...
        <n-modal
          v-model:show="showBatchOperationModal"
          :mask-closable="false"
        >
          <n-card
            style="width: 500px"
            :title="batchOperationType === 'enable' ? '批量启用通道' : '批量删除通道'"
            :bordered="false"
            size="huge"
            role="dialog"
            aria-modal="true"
          >
            <div class="space-y-4">
              <n-alert 
                ...
                :title="batchOperationType === 'enable' ? '确认启用' : '确认删除'"
              >
              ...
              <div class="max-h-60 overflow-y-auto">
                <n-list>
                  <n-list-item v-for="channelId in selectedDisabledChannels" :key="channelId">
                    <template #prefix>
                      <n-icon :color="batchOperationType === 'enable' ? '#18a058' : '#d03050'">
                        <component :is="batchOperationType === 'enable' ? CheckCircle : Warning" />
                      </n-icon>
                    </template>
            ...
            <template #footer>
              <div class="flex justify-end gap-3">
                <n-button @click="showBatchOperationModal = false">取消</n-button>
                <n-button 
                  :type="batchOperationType === 'enable' ? 'success' : 'error'"
      ...
      <n-tab-pane name="third-party-channels" tab="三方支付通道">
        ...
        <n-card class="mb-4">
          <div class="filter-section">
            ...
            <div class="filter-row">
              ...
              />
              <n-button type="primary" @click="handleChannelSearch">搜索</n-button>
              <n-button @click="handleChannelReset">重置</n-button>
              <div class="ml-auto">
                <n-button type="primary" @click="handleAddChannel">新增三方</n-button>
              </div>
            </div>
          </div>
        </n-card>
            ...
            <n-card :bordered="false" class="rounded-16px shadow-sm">
              <div class="flex justify-between items-center">
                ...
                <div class="flex items-center gap-4">
                  <!-- 选择控制 -->
                  <div class="flex gap-2">
                    <n-button size="small" @click="handleChannelReset">重置筛选</n-button>
                    <n-button size="small" @click="fetchChannelData">刷新数据</n-button>
                  </div>
                </div>
              </div>
            </n-card>
        ...
        >
          ...
          >
            ...
            <div style="max-height: calc(95vh - 180px); overflow-y: auto; padding: 20px;">
              <n-form 
                ref="channelFormRef" 
                :model="channelForm" 
                :rules="channelRules" 
              ...
              >
                ...
                />
                                    ...
                                    <n-form-item label="第三方支付" path="thirdPartyPayment" required>
                <n-select
                ...
                <div class="grid grid-cols-2 gap-6 mb-6">
                  <n-form-item label="第三方支付平台名" path="platformName" required>
                    <n-input
              ...
              <n-form-item label="商户密钥" path="merchantKey">
                ...
                <div class="bg-gray-50 p-4 rounded-lg mb-6">
                  <div class="flex items-center justify-between mb-4">
                    ...
                    <n-switch v-model:value="channelConfigEnabled" :round="false">
                  ...
                  <div class="grid grid-cols-2 gap-4 mb-4">
                    <n-form-item label="通道所属大类" path="channelCategory" required>
                <n-select
                ...
                />
                  ...
                  <div class="flex gap-2 mb-4">
                    <n-button size="small" type="primary" @click="showFeeReductionModal = true">配置手续费减免</n-button>
                    <n-button size="small" type="info" @click="showBonusConfigModal = true">配置充值赠送</n-button>
                    <n-button size="small" type="warning" @click="showRecommendedAmountModal = true">配置推荐金额</n-button>
                  </div>
          ...
          <template #footer>
              ...
              <n-button type="primary" @click="handleSaveChannel" :loading="channelModalLoading">
                  确认
              </n-button>
            </div>
          </template>
        ...
        >
          <div style="max-height: 70vh; overflow-y: auto; padding: 0 16px;">
            <div class="mb-6 p-4 bg-blue-50 border-l-4 border-blue-500 rounded">
              <div class="text-sm text-blue-800">
                <strong>配置说明：</strong> 设置充值赠送规则，包括气泡显示、颜色、会员层级策略和金额区间配置
              </div>
            </div>
          <n-form ref="bonusFormRef" :model="bonusForm" :rules="bonusRules" label-placement="left" label-width="140px" class="space-y-6">
              <!-- Bonus Display Options -->
              <n-form-item label="赠送气泡" path="showBubble" class="mb-6">
                <n-radio-group v-model:value="bonusForm.showBubble" class="w-full">
              ...
              <n-form-item label="气泡背景颜色" path="bubbleColor" class="mb-6">
                <n-radio-group v-model:value="bonusForm.bubbleColor" class="w-full">
              ...
              <n-form-item label="会员层级" path="memberLevel" class="mb-6">
                <n-radio-group v-model:value="bonusForm.memberLevel" class="w-full">
                  <n-radio value="no-limit" class="mr-8">不区分层级</n-radio>
                  <n-radio value="by-level" class="mr-8">按层级分别配置充值赠送</n-radio>
                </n-radio-group>
                <div class="text-xs text-gray-500 mt-1">
              ...
              <div class="space-y-4 mb-6">
                <div class="text-sm font-medium text-gray-700 mb-3">充值金额区间配置</div>
                <div v-for="(range, index) in bonusForm.amountRanges" :key="index" class="flex items-center gap-4 p-4 border rounded bg-gray-50 hover:bg-gray-100 transition-colors">
                  <div class="flex items-center gap-2">
                    <span class="text-sm font-medium w-44 text-gray-700">{{ range.minAmount }} 到 {{ range.maxAmount }} R$</span>
                  </div>
                  <div class="flex items-center gap-2">
                    <span class="text-xs text-gray-500 w-20">固定金额:</span>
              <n-input-number
                ...
                :precision="2"
                  ...
                  <div class="flex items-center gap-2">
                    <span class="text-xs text-gray-500 w-16">比例:</span>
                    <n-input-number
                  ...
                  <n-button size="tiny" @click="removeAmountRange(index)" v-if="bonusForm.amountRanges.length > 1" class="ml-auto">
                    <template #icon>
                      <n-icon><Close /></n-icon>
                    </template>
                  </n-button>
                </div>
                <n-button type="primary" dashed size="small" @click="addAmountRange" class="mt-2">
                  <template #icon>
                    <n-icon><Plus /></n-icon>
                  </template>
                  添加金额区间
              ...
              <n-form-item label="赠送次数上限" class="mb-6">
                <n-radio-group v-model:value="bonusForm.dailyLimitType" class="w-full mb-3">
          ...
          <template #footer>
            <div class="flex justify-end gap-3 px-4 py-2">
              <n-button size="large" @click="handleCloseBonusModal">取消</n-button>
              <n-button type="primary" size="large" @click="handleSaveBonusConfig" :loading="bonusModalLoading">
                确认
              </n-button>
            </div>
          </template>
        ...
        <n-modal
          ...
          style="width: 600px"
        >
          <n-form ref="feeReductionFormRef" :model="feeReductionForm" label-placement="left" label-width="140px">
        ...
        >
          <div style="max-height: 70vh; overflow-y: auto;">
            <n-form ref="recommendedAmountFormRef" :model="recommendedAmountForm" label-placement="left" label-width="100px">
              ...
              <div class="mb-4">
                <div class="grid grid-cols-2 gap-6">
                  <!-- Left Column -->
                  <div>
                    <h4 class="font-medium mb-3">推荐金额设置</h4>
                    <div class="space-y-3">
                      <div v-for="(amount, index) in recommendedAmountForm.amounts.slice(0, 4)" :key="index" class="flex items-center gap-3">
                        <div class="flex items-center gap-2">
                          <span class="text-sm w-20">推荐金额(R$)</span>
                          <n-input
                  ...
                  <div>
                    <div class="space-y-3 mt-8">
                      <div v-for="(amount, index) in recommendedAmountForm.amounts.slice(4, 8)" :key="index + 4" class="flex items-center gap-3">
                        <div class="flex items-center gap-2">
                          <span class="text-sm w-20">推荐金额(R$)</span>
                          <n-input
                            v-model:value="amount.amount"
              ...
              <div class="text-xs text-gray-500 mb-4">
                说明: 会员按推荐金额充值才可获对应加赠，加赠金额与充值赠送金额加赠合出。
              </div>
            </n-form>
          </div>
          ...
          <template #footer>
            <div class="flex justify-end gap-2">
              <n-button @click="showRecommendedAmountModal = false">取消</n-button>
      ...
      <n-tab-pane name="third-party-statistics" tab="三方统计">
          ...
          <div class="filter-section">
            ...
            <div class="filter-row">
              <n-space align="center">
                ...
                <n-button type="primary" @click="handleStatsSearch">搜索</n-button>
          ...
          <template #header>
            <div class="flex items-center justify-between">
              ...
              <n-space>
                ...
                <n-button type="primary" size="small" @click="handleRefreshStats">
                  <template #icon>
              ...
              <n-card :bordered="false" class="rounded-16px shadow-sm">
                <div class="flex justify-between items-center">
                  <div class="flex items-center gap-4">
                    ...
                    <!-- 统计信息 -->
                    <div class="text-sm text-gray-600">
                      统计时间: {{ formatStatsDateRange() }}，共 {{ statsPaginationReactive.total }} 条记录
                    </div>
                  </div>
        ...
        <div class="grid grid-cols-4 gap-4 mt-4">
          <n-card size="small">
            <n-statistic label="总交易笔数" :value="statsSummary.totalTransactions">
              <template #suffix>笔</template>
            </n-statistic>
          </n-card>
          <n-card size="small">
            <n-statistic label="成功交易笔数" :value="statsSummary.successTransactions">
              <template #suffix>笔</template>
            </n-statistic>
          </n-card>
          <n-card size="small">
            <n-statistic label="总交易金额" :value="statsSummary.totalAmount" :precision="2">
          ...
          <n-card size="small">
            <n-statistic 
              ...
              :precision="2"
              :value-style="{ color: statsSummary.overallSuccessRate >= 95 ? '#18a058' : statsSummary.overallSuccessRate >= 85 ? '#f0a020' : '#d03050' }"
            >
    ...
    <n-modal v-model:show="showPresetOrderModal" :mask-closable="false">
      ...
      >
        ...
        <n-form :model="presetOrderForm" label-placement="left" label-width="120px" require-mark-placement="right-hanging">
          ...
          <div v-if="selectedMember" class="mb-4 p-3 bg-blue-50 border border-blue-200 rounded text-sm">
            <div class="text-xs text-blue-600 font-medium mb-2">已选择会员信息</div>
            <div class="space-y-1">
              <div class="flex justify-between">
                <span class="text-gray-600">会员ID:</span>
                <span class="font-medium text-gray-900">{{ selectedMember.value }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">账号:</span>
                <span class="font-medium text-gray-900">{{ selectedMember.account }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">姓名:</span>
                <span class="font-medium text-gray-900">{{ selectedMember.name || 'N/A' }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">VIP等级:</span>
                <span class="font-medium text-gray-900">{{ selectedMember.vipLevel }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">余额:</span>
                <span class="font-medium text-green-600">{{ formatCurrency(selectedMember.balance) }}</span>
              </div>
            </div>
          </div>
        </n-form>
        ...
        <template #footer>
          <div style="display: flex; justify-content: flex-end; gap: 12px;">
            ...
            <n-button type="primary" :loading="presetOrderLoading" @click="handleConfirmPresetOrder">
              确定
            </n-button>
          </div>
        </template>
    ...
    <n-modal v-model:show="showSupplementOrderModal" :mask-closable="false">
      ...
      >
        ...
        <n-form :model="supplementOrderForm" label-placement="left" label-width="120px" require-mark-placement="right-hanging">
          <n-form-item label="会员ID" path="memberId" required>
            <n-auto-complete
        ...
        <template #footer>
          <div style="display: flex; justify-content: flex-end; gap: 12px;">
            ...
            <n-button type="primary" :loading="supplementOrderLoading" @click="handleConfirmSupplementOrder">
              确定
            </n-button>
          </div>
        </template>
    ...
    <n-modal v-model:show="showCreateOrderModal" :mask-closable="false">
      ...
      >
        ...
        <n-form :model="createOrderForm" label-placement="left" label-width="120px" require-mark-placement="right-hanging">
          <!-- Member Search -->
          <n-form-item label="会员账号/ID" path="memberAccount" required>
            <n-input-group>
              ...
              />
              <n-button type="primary" @click="handleSearchMember" :loading="searchingMember">
                搜索
              </n-button>
            </n-input-group>
          </n-form-item>
          ...
          <div v-if="foundMember" class="mb-4 p-4 bg-gray-50 rounded-lg">
            <div class="grid grid-cols-3 gap-4 text-sm">
              <div>
                <span class="text-gray-600">会员ID:</span>
                <span class="font-medium ml-2">{{ foundMember.id }}</span>
              </div>
              <div>
                <span class="text-gray-600">会员账号:</span>
                <span class="font-medium ml-2">{{ foundMember.account }}</span>
              </div>
              <div>
                <span class="text-gray-600">会员币种:</span>
                <span class="font-medium ml-2">{{ foundMember.currency || 'BRL' }}</span>
              </div>
            </div>
          </div>
          ...
          <div v-if="foundMember">
            ...
            <n-form-item label="充值大类" path="rechargeCategory" required>
              ...
              />
              <div v-if="!rechargeCategoryOptions.length" class="text-red-500 text-sm mt-1">
                请选择充值大类
              </div>
            </n-form-item>

            <!-- 支付通道 (只有选择充值大类后才显示) -->
            <n-form-item v-if="createOrderForm.rechargeCategory" label="支付通道" path="selectedChannel" required>
              <n-select
              ...
              />
              <div v-if="availableChannelOptions.length === 0 && !loadingChannels" class="text-red-500 text-sm mt-1">
                该大类下暂无可用的支付通道
              </div>
            </n-form-item>
        ...
        <template #footer>
          <div style="display: flex; justify-content: flex-end; gap: 12px;">
            ...
            <n-button 
              ...
              :disabled="!foundMember || !createOrderForm.rechargeCategory || !createOrderForm.orderAmount || !createOrderForm.verificationPassword"
      ...
      >
        ...
        <div v-if="currentOrderDetail" class="order-detail">
          ...
          <n-descriptions bordered :column="2" title="基本信息">
            <n-descriptions-item label="订单号">{{ currentOrderDetail.orderId }}</n-descriptions-item>
            <n-descriptions-item label="订单状态">
              <n-tag :type="currentOrderDetail.status === 'SUCCESS' ? 'success' : 'warning'">
                {{ currentOrderDetail.status === 'SUCCESS' ? '充值成功' : currentOrderDetail.status }}
              </n-tag>
            </n-descriptions-item>
            <n-descriptions-item label="会员ID">{{ currentOrderDetail.user?.userID || currentOrderDetail.memberId }}</n-descriptions-item>
            <n-descriptions-item label="会员账号">{{ currentOrderDetail.memberAccount }}</n-descriptions-item>
            <n-descriptions-item label="会员层级">{{ currentOrderDetail.user?.memberTier?.tierName || '-' }}</n-descriptions-item>
            <n-descriptions-item label="充值大类">{{ currentOrderDetail.thirdPartyPayment }}</n-descriptions-item>
            <n-descriptions-item label="通道名称">{{ currentOrderDetail.channel?.channelName || currentOrderDetail.channelName }}</n-descriptions-item>
            <n-descriptions-item label="通道编码">{{ currentOrderDetail.channelCode }}</n-descriptions-item>
          </n-descriptions>

          <!-- Amount Info -->
          <n-descriptions bordered :column="2" title="金额信息" class="mt-4">
            <n-descriptions-item label="充值通道币种">{{ currentOrderDetail.channelCurrency || currentOrderDetail.channel?.currency || 'BRL' }}</n-descriptions-item>
            <n-descriptions-item label="数量">{{ formatCurrency(currentOrderDetail.channelAmount || currentOrderDetail.rechargeAmount) }}</n-descriptions-item>
            <n-descriptions-item label="汇率">{{ currentOrderDetail.exchangeRate ? `1:${currentOrderDetail.exchangeRate}` : '-' }}</n-descriptions-item>
            <n-descriptions-item label="会员币种">BRL</n-descriptions-item>
            <n-descriptions-item label="订单金额">{{ formatCurrency(currentOrderDetail.rechargeAmount) }}</n-descriptions-item>
            <n-descriptions-item label="赠送金额">{{ formatCurrency(currentOrderDetail.bonusAmount || 0) }}</n-descriptions-item>
            <n-descriptions-item label="总上分金额">{{ formatCurrency(currentOrderDetail.rechargeAmount + (currentOrderDetail.bonusAmount || 0)) }}</n-descriptions-item>
            <n-descriptions-item label="通道费率">{{ currentOrderDetail.channelFeeRate ? `${(currentOrderDetail.channelFeeRate * 100).toFixed(2)}%` : '-' }}</n-descriptions-item>
          </n-descriptions>

          <!-- Time Info -->
          <n-descriptions bordered :column="2" title="时间信息" class="mt-4">
            <n-descriptions-item label="创建时间">{{ formatDateTime(currentOrderDetail.createdAt) }}</n-descriptions-item>
            <n-descriptions-item label="成功时间">
              {{ (currentOrderDetail.status === 'SUCCESS' || currentOrderDetail.status === 'success') 
                  ? formatDateTime((currentOrderDetail as any).processingTime || (currentOrderDetail as any).confirmTime || (currentOrderDetail as any).completeTime)
                  : '-' }}
            </n-descriptions-item>
            <n-descriptions-item label="更新时间">{{ formatDateTime(currentOrderDetail.updatedAt) }}</n-descriptions-item>
            <n-descriptions-item label="操作人">{{ (currentOrderDetail as any).操作人 || 'system' }}</n-descriptions-item>
          </n-descriptions>

          <!-- Additional Info -->
          <n-descriptions bordered :column="1" title="其他信息" class="mt-4">
            <n-descriptions-item label="三方订单号">{{ currentOrderDetail.thirdPartyOrderId || '-' }}</n-descriptions-item>
            <n-descriptions-item label="标记">{{ (currentOrderDetail as any).标记 || '-' }}</n-descriptions-item>
            <n-descriptions-item label="是否首存">{{ (currentOrderDetail as any).是否首存 || '否' }}</n-descriptions-item>
            <n-descriptions-item label="备注">{{ currentOrderDetail.remark || '-' }}</n-descriptions-item>
          </n-descriptions>

          <!-- Error Details (if applicable) -->
          <div v-if="currentOrderDetail.errorDetails || currentOrderDetail.errorCode" class="mt-4">
            <n-descriptions bordered :column="1" title="错误信息">
              <n-descriptions-item v-if="currentOrderDetail.errorCode" label="错误代码">{{ currentOrderDetail.errorCode }}</n-descriptions-item>
              <n-descriptions-item v-if="currentOrderDetail.errorDetails" label="错误详情">{{ currentOrderDetail.errorDetails }}</n-descriptions-item>
            </n-descriptions>
          </div>
        </div>
    ...
    <n-modal v-model:show="showColumnConfig" :mask-closable="false">
      ...
      >
        <div class="column-config-content">
          <p class="text-gray-600 mb-4">拖拽重新排序，勾选控制显示</p>
...
import {
  NCard, NTabs, NTabPane, NDataTable, NButton, NInput, NSelect, NDatePicker, NDropdown, NSwitch,
  NTag, NText, NSpace, NBreadcrumb, NBreadcrumbItem, NTooltip, NModal, NForm, NFormItem,
  NInputNumber, NInputGroup, NInputGroupLabel, NIcon, NAutoComplete, NCheckbox, NCheckboxGroup,
  NEmpty, NAlert, NList, NListItem, NPopconfirm, NStatistic, NSpin, NPagination, NRadio, NRadioGroup,
  NDescriptions, NDescriptionsItem,
  useMessage, useDialog, type DataTableColumns, type DropdownOption
} from 'naive-ui';
import { 
  Close, 
  Search, 
...
const statsColumns: DataTableColumns<any> = [
  {
    ...
    width: 150,
    ellipsis: true,
    render: (row: any) => h('div', { class: 'font-medium text-blue-600' }, row.platformName || '-'),
  },
  {
    title: '通道所属大类',
    key: 'channelCategory',
    width: 120,
    render: (row: any) => h('n-tag', { type: 'info', size: 'small' }, { default: () => row.channelCategory }),
  },
  {
    title: '三方通道名称/编码',
    key: 'channelInfo',
  ...
  },
  {
    ...
    render: (row: any) => h('span', { class: 'font-medium' }, row.uniqueUsers || 0),
  },
  {
    title: '合计笔数',
    key: 'totalOrders',
    width: 80,
    align: 'center' as const,
    render: (row: any) => h('span', { class: 'font-medium' }, row.totalOrders || 0),
  },
  {
    title: '成功笔数',
    key: 'successOrders',
    width: 80,
    align: 'center' as const,
    render: (row: any) => h('span', { class: 'font-medium text-green-600' }, row.successOrders || 0),
  },
  {
    title: '成功金额',
    key: 'successAmount',
  ...
  },
  {
    ...
    render: (row: any) => {
      ...
      return h('div', { class: 'text-xs' }, [
        ...
        }, isPositive ? `+${trend.toFixed(2)}` : trend < 0 ? trend.toFixed(2) : '0.00'),
        h('div', { class: 'text-gray-500' }, 'BRL'),
      ]);
    },
  },
...
const disabledChannelColumns: DataTableColumns<any> = [
  ...
  },
  {
    ...
    width: 150,
    ellipsis: true,
    render: (row: any) => h('div', { class: 'font-medium' }, row.channelName || '-'),
  },
  {
    title: '支付平台',
    key: 'platformName',
  ...
  },
  {
    ...
    width: 120,
    ellipsis: true,
    render: (row: any) => h('n-tag', { type: 'info', size: 'small' }, { default: () => row.channelCode }),
  },
  {
    title: '币种',
    key: 'currency',
    width: 80,
    render: (row: any) => h('n-tag', { type: 'default', size: 'small' }, { default: () => row.currency }),
  },
  {
    title: '交易限额',
    key: 'limits',
  ...
  },
  {
    ...
    key: 'memberLevel',
    width: 100,
    render: (row: any) => h('n-tag', { type: 'warning', size: 'small' }, { default: () => row.memberLevel || 'ALL' }),
  },
  {
    title: '停用时间',
    key: 'updatedAt',
    ...
    render: (row: any) => {
      ...
      return h('div', { class: 'text-xs text-gray-600' }, new Date(row.updatedAt).toLocaleString('zh-CN'));
    },
  },
  {
    title: '停用原因',
    ...
    render: (row: any) => {
      ...
      return h('n-tooltip', { trigger: 'hover' }, {
        trigger: () => h('span', { class: 'cursor-help text-blue-600' }, row.remark.slice(0, 20) + (row.remark.length > 20 ? '...' : '')),
        default: () => row.remark,
      });
    },
  },
    ...
    render: (row: any) => {
      return h('n-space', { size: 'small' }, [
        h('n-button', 
          ...
          { default: () => '启用', icon: () => h('n-icon', null, { default: () => h('CheckCircle') }) }
        ),
        h('n-button', 
          { 
            type: 'info', 
          ...
          { default: () => '编辑', icon: () => h('n-icon', null, { default: () => h('Edit') }) }
        ),
        h('n-popconfirm', 
          {
          ...
          },
          {
            trigger: () => h('n-button', 
              ...
              { default: () => '删除', icon: () => h('n-icon', null, { default: () => h('Trash') }) }
            ),
            default: () => '确定要永久删除此通道吗？',
          }
        ),
...
const bonusForm = ref({
  ...
  amountRanges: [
    ...
    { minAmount: '1000', maxAmount: '2999.99', fixedAmount: '', bonusRate: 0.4 },
    { minAmount: '3000', maxAmount: '9999.99', fixedAmount: '', bonusRate: 0.6 },
    { minAmount: '10000', maxAmount: '29999.99', fixedAmount: '', bonusRate: 2.6 },
    { minAmount: '30000', maxAmount: '50000.00', fixedAmount: '', bonusRate: 8.8 }
  ],
  bonusLimit: '',
...
const channelRules = {
  ...
  currency: [
    { required: true, message: '请选择通道币种', trigger: 'change' }
  ],
  'channelLimit.min': [
    { 
      required: true, 
...
const categoryRules = {
  ...
  badge: [
    { max: 12, message: '大类角标不能超过12个字符', trigger: 'blur' }
  ]
};
...
const convertToCSV = (data: RechargeOrder[]): string => {
  const headers = [
    '订单ID', '会员ID', '会员账号', '会员姓名', 'VIP等级', '充值金额', '币种',
    '第三方支付', '通道名称', '商户ID', '订单类型', '状态', '提交时间',
    '支付时间', '完成时间', '实际金额', '赠送金额', '手续费', '备注'
  ];
...
const columns: DataTableColumns<RechargeOrder> = [
  ...
  },
  {
    ...
    width: 140,
    render: (row) => {
      const successTime = (row.status === 'SUCCESS' || row.status === 'success') 
        ? ((row as any).processingTime || (row as any).confirmTime || (row as any).completeTime)
        : '';
      return h(
      NText,
      { style: { fontSize: '12px' } },
  ...
  },
  {
    ...
    render: (row) => h(
      ...
      { default: () => row.user?.memberTier?.tierName || row.memberTier || '默认层级' }
    )
  },
  {
    title: '充值通道币种',
    ...
    render: (row) => h(
      ...
      { class: 'text-xs' },
      [
        h('div', { class: 'font-medium' }, row.channelCurrency || row.channel?.currency || row.currency),
  ...
  },
  {
    ...
    render: (row) => h(
      ...
      { class: 'text-xs' },
      [
        h('div', { class: 'font-medium' }, formatCurrency(row.channelAmount || row.rechargeAmount)),
        h('div', { class: 'text-gray-500' }, row.channelCurrency || row.currency)
      ]
    )
  },
  {
    ...
    render: (row) => {
      ...
      return h(
        ...
        [
          ...
          h('div', { class: 'text-gray-500' }, `${row.currency}:${row.channelCurrency}`)
        ]
      );
    }
  },
  {
    ...
    render: (row) => h(
      ...
      [
        ...
        row.conversionRatio ? h('div', { class: 'text-gray-500' }, `比例1:${row.conversionRatio}`) : null
      ].filter(Boolean)
    )
  },
  {
    ...
    render: (row) => h(
      ...
      { default: () => formatCurrency((row as any).orderAmount || row.rechargeAmount) }
    )
  },
  {
    title: '赠送金额',
  ...
  },
  {
    ...
    render: (row) => h(
      ...
      { default: () => formatCurrency((row as any).totalCredited || (row.rechargeAmount + row.bonusAmount)) }
    )
  },
  {
    title: '第三方支付',
  ...
  },
  {
    ...
    render: (row) => {
      ...
      if ((row.status === 'FAILED' || row.status === 'failed' || row.status === 'CANCELLED' || row.status === 'canceled' || row.status === 'expired') && (row.errorDetails || row.errorCode)) {
        elements.push(
          h(
            NButton,
            {
              ...
              onClick: () => {
                ...
                dialog.info({
                  title: '错误详情',
                  content: () => h('div', { class: 'text-sm' }, [
                    row.errorCode ? h('div', { class: 'mb-2' }, [
                      h('strong', '错误代码: '),
                      h('span', row.errorCode)
                    ]) : null,
                    row.errorDetails ? h('div', [
                      h('strong', '错误信息: '),
  ...
  },
  {
    ...
    render: (row: any) => {
      ...
      const remarksRaw = (row as any).remarks || (row as any).remark || ((row as any).metadata && (row as any).metadata.remarks) || '';
      ...
      const remarks = TRANSACTION_SUBCATEGORY_MAPPINGS[remarksRaw] || remarksRaw;
  ...
  },
  {
    ...
    render: (row) => {
      ...
      const hasForceOperation = (row as any).metadata?.forceCancelledBy || 
                               (row as any).metadata?.forceDepositRequestedBy ||
                               ['SUCCESS', 'CANCELLED', 'CANCELED'].includes(status);
      
      if (hasForceOperation || ['SUCCESS', 'CANCELLED', 'CANCELED'].includes(status)) {
        actions.push(
          h(
            NButton,
            {
...
const channelColumns: DataTableColumns<ThirdPartyChannel> = [
  ...
  },
  {
    ...
    render: (row) => h(
      NText,
      { style: { fontSize: '12px' } },
      { default: () => row.ipAddress && row.ipAddress.length > 12 ? `${row.ipAddress.substring(0, 12)}...` : row.ipAddress || '-' }
    )
  },
  {
    title: '通道名称',
  ...
  },
  {
    ...
    render: (row) => h(
      NText,
      { style: { fontSize: '12px' } },
      { default: () => row.channelLimit ? `${row.channelLimit.min || 0} ~ ${row.channelLimit.max || 0}` : '未设置' }
    )
  },
  {
    title: '赠送比例(%)',
      ...
      { size: 'small' },
      {
        default: () => [
          h(
            NText,
            { style: { fontSize: '12px' } },
            { default: () => (typeof row.bonusRate === 'number' ? row.bonusRate.toFixed(2) : Number(row.bonusRate || 0).toFixed(2)) }
          ),
          h(
            NButton,
            {
  ...
  },
  {
    ...
    render: (row) => h(
      ...
      { size: 'small' },
      {
        default: () => (row.terminal && Array.isArray(row.terminal) ? row.terminal : []).map(terminal => 
          h(
            NTag,
  ...
  },
  {
    ...
    render: (row) => h(
      NSwitch,
      {
        ...
        size: 'small',
        'onUpdate:value': (value: boolean) => handleToggleChannelStatus(row, value)
      }
    )
  },
  {
    ...
    render: (row) => h(
      NText,
      { style: { fontSize: '12px' } },
      { default: () => row.successRate ? `${(row.successRate * 100).toFixed(1)}%` : '-' }
    )
  },
  {
    title: '处理时间(s)',
    ...
    render: (row) => h(
      ...
      [
        row.icon ? h('img', {
          ...
          class: 'w-6 h-6 rounded mr-2 bg-gray-200 flex items-center justify-center text-xs'
        }, '📁'),
        h(NText, { style: { fontSize: '12px' } }, { default: () => row.name || 'N/A' })
      ]
    )
  },
  {
  ...
  },
  {
    ...
    render: (row) => h(
      NSwitch,
      {
        ...
        size: 'small',
        'onUpdate:value': (value: boolean) => handleToggleCategoryStatus(row, value)
      }
    )
  },
  {
    ...
    render: (row) => h(
      ...
      { style: { fontSize: '12px' } },
      { default: () => {
        if (row.createdAt && typeof row.createdAt === 'object' && row.createdAt.toISOString) {
    ...
    try {
      ...
      if (statistics) {
        backendTotals.value = {
          ...
          orderAmount: statistics.totalConfirmedAmount || statistics.totalAmount || 0, // 订单金额合计
      ...
      if (tableData.value.length > 0) {
        console.log('🎁 First order bonusAmount:', tableData.value[0].bonusAmount);
...
const loadRechargeCategories = async () => {
  try {
    ...
    if (actualData.list && actualData.list.length > 0) {
      ...
      console.log('✅ Loaded recharge category options:', rechargeCategoryOptions.value);
    } else {
...
const handleRechargeCategoryChange = async (value: string | number) => {
  ...
  const selectedCategory = rechargeCategoryOptions.value.find(option => option.value === value);
  if (selectedCategory) {
...
const loadAvailableChannels = async (categoryName: string) => {
  ...
  try {
    ...
    if (response && response.data && response.data.records) {
      ...
      const filteredChannels = response.data.records.filter((channel: any) => 
...
const handleChannelChange = (value: string) => {
  ...
  const selectedOption = availableChannelOptions.value.find(option => option.value === value);
  if (selectedOption) {
...
const handleConfirmCreateOrder = async () => {
  ...
  try {
    ...
    const orderData = {
      ...
      thirdPartyPayment: channelDetails.thirdPartyPayment || selectedCategory.name,
      ...
      bonusAmount: createOrderForm.bonusValue ? parseFloat(createOrderForm.bonusValue) : 0,
      fees: 0,
    ...
    if (error && typeof error === 'object' && error.id && error.orderId) {
      ...
      showCreateOrderModal.value = false;
      await fetchData();
      console.log('✅ Order created successfully via error handler:', error.orderId);
    } else {
...
const handleMemberInput = (value: string) => {
  ...
  if (selectedMember.value) {
    const currentLabel = selectedMember.value.account + ' - ' + selectedMember.value.name;
...
const handleConfirmSupplementOrder = async () => {
  try {
    ...
    console.log('✅ selectedMember.value:', selectedMember.value);
    console.log('✅ supplementOrderForm.memberId:', supplementOrderForm.value.memberId);
...
const calculateBonusAmount = (order: RechargeOrder) => {
  ...
  if (order.channel?.bonusConfig) {
    try {
      const bonusConfig = typeof order.channel.bonusConfig === 'string' 
...
const handleBulkProcess = (selectedRows?: RechargeOrder[]) => {
  const orders = selectedRows || tableData.value.filter(item => checkedRowKeys.value.includes(item.orderId));
...
const handleForceDeposit = async (row: any) => {
  try {
    ...
    const response = await requestClient.put(`/transactions/deposits/${row.id}/force`, {
...
const handleForceCancel = async (row: any) => {
  try {
    ...
    const response = await requestClient.put(`/transactions/deposits/${row.id}/cancel`, {
      reason: reason
    });
...
const handleUpdateRemarks = async (row: any) => {
  try {
    ...
    const remarksInput = await new Promise<string | null>((resolve) => {
      const currentRemarks = row.remarks || (row.metadata && row.metadata.remarks) || '';
      const inputRef = ref(currentRemarks);
      const d = dialog.create({
        title: '添加备注',
        content: () => {
          return h('div', [
            h('p', { style: { marginBottom: '12px', color: '#666' } }, `订单号: ${row.orderId}`),
            h(NInput, {
    ...
    if (remarksInput !== null && remarksInput.trim()) {
      ...
      const response = await requestClient.put(`/transactions/deposits/${row.id}/remarks`, {
...
const fetchChannelData = async () => {
  ...
  try {
    ...
    if (actualData.records) {
      ...
      console.log('✅ Channel pagination total:', channelPaginationReactive.total);
    } else {
...
const handleEditChannel = async (channel: any) => {
  ...
  channelForm.value = {
    ...
    memberLevels: Array.isArray(channel.memberLevels) ? channel.memberLevels : [],
    ...
    recommendedAmounts: channel.recommendedAmounts || '10.00,15.00,50.00,100.00,300.00,500.00,1000.00,5000.00',
    notificationMessage: channel.notificationMessage || '',
    channelConfig: {
      enabled: channel.channelConfig?.enabled !== undefined ? channel.channelConfig.enabled : true
    },
    // Bonus configuration
  ...
  console.log('🔍 [PROD DEBUG] Channel recommendedAmounts type:', typeof channel.recommendedAmounts);
  console.log('🔍 [PROD DEBUG] Channel recommendedAmounts value:', channel.recommendedAmounts);
  console.log('🔍 [PROD DEBUG] Channel recommendedAmounts stringified:', JSON.stringify(channel.recommendedAmounts));
  ...
  if (channel.recommendedAmounts && typeof channel.recommendedAmounts === 'string') {
    const amounts = channel.recommendedAmounts.split(',')
    ...
    console.log('🔄 [PROD DEBUG] After string population:', JSON.stringify(recommendedAmountForm.value, null, 2));
  } else if (channel.recommendedAmounts && typeof channel.recommendedAmounts === 'object') {
    console.log('📝 [PROD DEBUG] Channel has structured recommendedAmounts:', JSON.stringify(channel.recommendedAmounts, null, 2));
    
    // Try to extract amounts from the object structure
    if (channel.recommendedAmounts.amounts && Array.isArray(channel.recommendedAmounts.amounts)) {
      console.log('🔍 [PROD DEBUG] Processing amounts array:', channel.recommendedAmounts.amounts);
      ...
      channel.recommendedAmounts.amounts.forEach((item: any, index: number) => {
        if (index < 8 && recommendedAmountForm.value.amounts[index]) {
          ...
          const amountValue = item.rewardAmount ?? item.amount ?? item.value ?? null;
          
          console.log(`🔍 [PROD DEBUG] Item ${index}:`, { item, amountValue, type: typeof amountValue });
      ...
      if (channel.recommendedAmounts.mode) {
        recommendedAmountForm.value.amountType = channel.recommendedAmounts.mode === 'FIXED' ? '仅限固定金额' : '可输入任意金额';
      }
    }
    
    console.log('🔄 [PROD DEBUG] After object population:', JSON.stringify(recommendedAmountForm.value, null, 2));
  } else {
    console.log('📝 [PROD DEBUG] No recommendedAmounts data found, using default form');
  }
  
  // ✅ FIX: Use nextTick to ensure form is mounted before logging final state
  await nextTick();
  console.log('✅ [PROD DEBUG] Final recommendedAmountForm.value:', JSON.stringify(recommendedAmountForm.value, null, 2));
  console.log('✅ [PROD DEBUG] Form amounts length:', recommendedAmountForm.value.amounts.length);
  recommendedAmountForm.value.amounts.forEach((item, idx) => {
    console.log(`✅ [PROD DEBUG] Form amount[${idx}]:`, { amount: item.amount, bonusType: item.bonusType, bonusValue: item.bonusValue });
  });
  ...
  if (channel.bonusConfig && typeof channel.bonusConfig === 'object') {
    ...
    bonusForm.value = {
      ...
      memberLevel: channel.bonusConfig.memberLevelStrategy === 'UNIFIED' ? 'no-limit' : 'by-level',
      amountRanges: [
        { minAmount: '1.00', maxAmount: '29.99', fixedAmount: '', bonusRate: 0.0 },
        { minAmount: '30', maxAmount: '999.99', fixedAmount: '', bonusRate: 0.2 },
        { minAmount: '1000', maxAmount: '2999.99', fixedAmount: '', bonusRate: 0.4 },
      ...
      bonusLimit: channel.bonusConfig.maxBonusAmount ? String(channel.bonusConfig.maxBonusAmount) : '',
      dailyLimitType: channel.bonusConfig.countLimitType === 'DAILY' ? 'daily' : 'total',
      dailyLimit: channel.bonusConfig.maxBonusCount ? String(channel.bonusConfig.maxBonusCount) : ''
    };
    
    // Populate amount ranges from bonus config
    if (channel.bonusConfig.bonusRules && Array.isArray(channel.bonusConfig.bonusRules)) {
      channel.bonusConfig.bonusRules.forEach((rule: any, index: number) => {
        if (index < 6 && bonusForm.value.amountRanges[index]) {
          bonusForm.value.amountRanges[index].minAmount = String(rule.minAmount);
          bonusForm.value.amountRanges[index].maxAmount = String(rule.maxAmount);
          bonusForm.value.amountRanges[index].bonusRate = rule.bonusRatio * 100; // Convert decimal to percentage
          bonusForm.value.amountRanges[index].fixedAmount = rule.fixedAmount ? String(rule.fixedAmount) : '';
        }
      });
    }
    
    console.log('🔄 Populated bonusForm from existing bonus config:', bonusForm.value);
  } else {
...
const handleSaveChannel = async () => {
  ...
  try {
    ...
    console.log('📊 Recommended amounts data type:', typeof channelForm.value.recommendedAmounts);
    console.log('📊 Recommended amounts data:', channelForm.value.recommendedAmounts);
    ...
    if (isEditingChannel.value && currentEditingChannel.value) {
      ...
      const response = await thirdPartyChannelApi.update(currentEditingChannel.value.id, backendData as any);
...
const handleConfigBonusRate = (channel: ThirdPartyChannel) => {
  ...
  if (channel.bonusConfig && typeof channel.bonusConfig === 'object') {
    ...
    bonusForm.value.showBubble = channel.bonusConfig.showBubble ? 'show' : 'hide';
    bonusForm.value.bubbleColor = channel.bonusConfig.bubbleBackgroundColor || 'blue';
    bonusForm.value.memberLevel = channel.bonusConfig.memberLevelStrategy === 'UNIFIED' ? 'no-limit' : 'by-level';
    bonusForm.value.bonusLimit = channel.bonusConfig.maxBonusAmount ? String(channel.bonusConfig.maxBonusAmount) : '';
    bonusForm.value.dailyLimitType = channel.bonusConfig.countLimitType === 'DAILY' ? 'daily' : 'total';
    bonusForm.value.dailyLimit = channel.bonusConfig.maxBonusCount ? String(channel.bonusConfig.maxBonusCount) : '';
...
const handleSaveBonusConfig = async () => {
  try {
    ...
    if (!bonusForm.value.amountRanges || bonusForm.value.amountRanges.length === 0) {
    ...
    const bonusConfig = {
      ...
      memberLevelStrategy: bonusForm.value.memberLevel === 'no-limit' ? 'UNIFIED' : 'SEPARATE_BY_LEVEL',
      bonusRules: bonusForm.value.amountRanges
        .filter((range: any) => {
          ...
          const isValid = !isNaN(minAmount) && !isNaN(maxAmount) && !isNaN(bonusRate) &&
                         minAmount > 0 && maxAmount > minAmount && bonusRate >= 0;
          
          if (!isValid) {
            console.log('⚠️ Filtering out invalid range:', { range, minAmount, maxAmount, bonusRate });
          }
        ...
        .map((range: any) => {
          ...
          if (minAmount <= 0 || maxAmount <= minAmount) {
            console.error('❌ Invalid range data detected:', { range, minAmount, maxAmount });
            throw new Error(`Invalid range: minAmount (${minAmount}) must be > 0 and maxAmount (${maxAmount}) must be > minAmount`);
          }
          ...
          return {
            ...
            fixedAmount: range.fixedAmount ? parseFloat(range.fixedAmount) : undefined
          };
        }),
      maxBonusAmount: bonusForm.value.bonusLimit ? parseFloat(bonusForm.value.bonusLimit) : undefined,
      maxBonusCount: bonusForm.value.dailyLimit ? parseInt(bonusForm.value.dailyLimit) : undefined,
      countLimitType: bonusForm.value.dailyLimitType === 'daily' ? 'DAILY' : 'TOTAL'
    };
    ...
    if (bonusConfig.bonusRules && bonusConfig.bonusRules.length > 0) {
      ...
      console.log('💾 Bonus config stored in channel form:', channelForm.value.bonusConfig);
    } else {
...
const handleSaveRecommendedAmount = async () => {
  try {
    ...
    const validAmountConfigs = recommendedAmountForm.value.amounts
      ...
      .filter((config: any) => !isNaN(config.rewardAmount) && config.rewardAmount > 0);
    
    // Create the structured configuration for database storage
    const recommendedAmountConfig = {
      mode: recommendedAmountForm.value.amountType === '仅限固定金额' ? 'FIXED' : 'FLEXIBLE',
    ...
    const displayAmounts = validAmountConfigs.map((config: any) => config.rewardAmount).join(',');
    
    // Update the display field to show amounts with bonus info
    const displayText = validAmountConfigs.map((config: any) => 
      `${config.rewardAmount} BRL (${config.bonusValue}%)`
    ).join(', ');
    
    console.log('🔄 Recommended amounts config saved:', recommendedAmountConfig);
...
const handleSaveBonusRate = async () => {
  try {
    ...
    console.log('💰 Updating bonus rate for channel:', currentBonusChannel.value.id);
...
const handleDeleteChannel = async (channel: any) => {
  try {
    const confirmed = await window.confirm(`确定要删除通道 "${channel.channelName}" 吗？`);
    if (!confirmed) return;
...
const handleDeleteCategory = async (category: any) => {
  try {
    const confirmed = await window.confirm(`确定要删除大类 "${category.name}" 吗？`);
    if (!confirmed) return;
...
const handleSaveCategory = async () => {
  ...
  try {
    ...
    if (isEditingCategory.value && currentEditingCategory.value) {
      ...
      const response = await updateRechargeCategory(currentEditingCategory.value.id, categoryForm.value);
...
const handleToggleCategoryStatus = async (category: any, value: boolean) => {
  try {
    ...
    // Revert the change if API call fails
    category.isActive = category.isActive === 'ENABLED' ? 'DISABLED' : 'ENABLED';
    message.error('状态更新失败');
...
const fetchDisabledChannelData = async () => {
  ...
  try {
    ...
    if (response && response.data && response.data.records) {
      ...
      disabledChannelPaginationConfig.value.itemCount = response.data.pagination?.total || 0;
      
      // Auto-expand first group if it exists
      if (response.data.records.length > 0) {
        const firstCategory = response.data.records[0].channelCategory || '未分类';
...
const getChannelDisplayName = (channelId: string) => {
  const channel = disabledChannelTableData.value.find(c => c.id === channelId);
  return channel ? `${channel.platformName} - ${channel.channelName}` : channelId;
};
...
const confirmBatchOperation = async () => {
  ...
  try {
    ...
    message.success(`批量${action}成功，共处理 ${selectedDisabledChannels.value.length} 个通道`);
...
const fetchStatsData = async () => {
  ...
  try {
    ...
    console.log('📊 Orders count:', response?.orders?.length || response?.data?.orders?.length || 0);
    ...
    if (response && orders && orders.length > 0) {
      ...
      const successStatuses = ['SUCCESS', 'success', 'COMPLETED', 'completed', 'CONFIRMED', 'confirmed'];
      ...
      orders.forEach((order: any) => {
        ...
        if (!channelStats.has(channelKey)) {
          channelStats.set(channelKey, {
            id: order.id,
            platformName: order.platformName || order.paymentGateway || order.paymentMethod || 'Unknown',
            channelCategory: order.channelCategory || order.paymentCategory || order.productCode || 'Unknown',
            channelName: order.channelName || order.paymentGateway || 'Unknown Channel',
            channelCode: order.channelCode || order.paymentMethod || order.productCode || 'UNKNOWN',
            totalOrders: 0,
            successOrders: 0,
            successAmount: 0,
            uniqueUsers: new Set(),
        ...
        if (successStatuses.includes(order.status)) {
          stats.successOrders++;
          stats.successAmount += Number(order.amount || order.rechargeAmount || 0);
        }
        ...
        if (orderTime >= thirtyMinutesAgo) {
          if (successStatuses.includes(order.status)) {
            ...
            stats.recent30MinAmount += Number(order.amount || order.rechargeAmount || 0);
          }
        }
      });
      ...
      const statsArray = Array.from(channelStats.values()).map(stats => ({
        ...
        successRate: stats.totalOrders > 0 ? (stats.successOrders / stats.totalOrders) * 100 : 0,
        recent30MinRate: stats.recent30MinSuccess > 0 ? (stats.recent30MinSuccess / Math.max(stats.totalOrders, 1)) * 100 : 0,
      ...
      if (statsFilters.platform) {
        filteredStats = statsArray.filter(stat => 
          stat.platformName.toLowerCase().includes(statsFilters.platform!.toLowerCase())
        );
      }
      if (statsFilters.category) {
        filteredStats = statsArray.filter(stat => 
          stat.channelCategory.toLowerCase().includes(statsFilters.category!.toLowerCase())
        );
      }
      ...
      // Calculate summary
      const totalTrans = filteredStats.reduce((sum, stat) => sum + stat.totalOrders, 0);
      const successTrans = filteredStats.reduce((sum, stat) => sum + stat.successOrders, 0);
      const totalAmount = filteredStats.reduce((sum, stat) => sum + stat.successAmount, 0);
      
      statsSummary.value = {
        totalTransactions: totalTrans,
        successTransactions: successTrans,
        totalAmount: totalAmount,
        overallSuccessRate: totalTrans > 0 ? (successTrans / totalTrans) * 100 : 0,
      };
      ...
      console.log('📊 No orders found in response:', { response, orders: orders?.length || 0 });
...
const handleExportStats = () => {
  ...
  const headers = [
    '三方支付', '通道所属大类', '通道名称', '通道编码', '充值人数', 
    '合计笔数', '成功笔数', '成功金额', '近30分钟成功笔数', '近30分钟成功金额',
    '成功率(%)', '近30分钟成功率(%)', '近30分钟金额上升', '近30分钟金额下降'
  ];
  ...
  link.setAttribute('download', `三方支付统计_${formatStatsDateRange().replace(/[\/\s]/g, '_')}.csv`);
...
const getCategoryIcon = (category: string) => {
  ...
  const categoryData = categoryTableData.value.find(cat => cat.name === category);
...
const getSelectedChannelsInGroup = (category: string) => {
  const group = groupedDisabledChannels.value.find(g => g.category === category);
  if (!group) return [];
...
watch(activeTab, () => {
  if (activeTab.value === 'all-orders') {
    ...
    console.log('📊 Third party statistics tab activated with filters:', statsFilters);
    if (statsFilters.startDate && statsFilters.endDate) {
...
const loadColumnConfig = async () => {
  try {
    const response = await requestClient.get(`/backoffice/preferences/${COLUMN_CONFIG_KEY}`);
...
const resetColumnConfig = async () => {
  try {
    ...
    const defaultVisible = [
      'orderId', 'memberAccount', 'firstDepositStatus', 'vipLevel', 'exchangeRate', 'currency',
      'rechargeAmount', 'bonusAmount', 'feeAmount', 'totalCreditAmount',
      'thirdPartyPayment', 'channelName', 'channelFeeRate', 'status',
      'submitTime', 'createdAt', 'updatedAt', 'user.memberTier.tierName'
    ];
...
const setupRealTimeUpdates = () => {
  ...
  console.log('📡 Real-time updates disabled for admin panel - admins perform actions manually');
  return;
