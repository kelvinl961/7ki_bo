      ...
      <div v-if="taskDetail" class="task-detail-content">
        ...
        >
          ...
          <n-descriptions-item label="支持平台">
            <n-space>
              <n-tag v-if="taskDetail.platforms.android" size="small">Android</n-tag>
          ...
          <n-descriptions-item label="领取入口">
            <n-space>
              <n-tag v-if="taskDetail.claimEntrance.androidApp" size="small">Android APP</n-tag>
              <n-tag v-if="taskDetail.claimEntrance.iosApp" size="small">iOS APP</n-tag>
          ...
          <n-descriptions-item label="任务条件">
            {{ taskDetail.taskConditions.conditionType }}: {{ taskDetail.taskConditions.targetValue }}
          </n-descriptions-item>
          ...
          <n-descriptions-item v-if="taskDetail.description" label="任务描述" :span="2">
            {{ taskDetail.description }}
          </n-descriptions-item>
          
          <n-descriptions-item v-if="taskDetail.ruleDescription" label="规则说明" :span="2">
            <pre style="white-space: pre-wrap; font-family: inherit;">{{ taskDetail.ruleDescription }}</pre>
          </n-descriptions-item>
        </n-descriptions>
        
        <!-- 最近参与用户 -->
        <div v-if="taskDetail.recentParticipants && taskDetail.recentParticipants.length > 0" class="participants-section">
...
const getRewardTypeColor = (rewardType: string) => {
  switch (rewardType) {
    ...
    case 'POINTS': return 'info';
    case 'BONUS': return 'warning';
    case 'FREE_SPINS': return 'primary';
