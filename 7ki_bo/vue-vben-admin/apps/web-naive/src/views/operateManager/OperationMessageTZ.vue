      ...
      <div v-if="detailData" class="detail-content">
        <div class="grid grid-cols-2 gap-4 mb-4">
          ...
          <div>
            ...
            <div class="mt-1 p-2 bg-gray-50 rounded">{{ detailData.receiverType }}</div>
          </div>
          <div>
            <label class="text-sm font-medium text-gray-600">触发条件</label>
            <div class="mt-1 p-2 bg-gray-50 rounded">{{ detailData.triggerCondition }}</div>
          </div>
          <div>
            <label class="text-sm font-medium text-gray-600">状态</label>
            <div class="mt-1 p-2 bg-gray-50 rounded">
              <n-tag :type="detailData.status === '已启用' ? 'success' : 'error'">
                {{ detailData.status }}
              </n-tag>
            </div>
          </div>
        ...
        <div class="grid grid-cols-3 gap-4 mb-4">
          <div>
            <label class="text-sm font-medium text-gray-600">弹窗入口</label>
            <div class="mt-1 p-2 bg-gray-50 rounded">{{ detailData.popupEntrance ? '是' : '否' }}</div>
          </div>
          <div>
            <label class="text-sm font-medium text-gray-600">视频推送</label>
            <div class="mt-1 p-2 bg-gray-50 rounded">{{ detailData.videoPush ? '是' : '否' }}</div>
          </div>
          <div>
            <label class="text-sm font-medium text-gray-600">已读/发送</label>
            <div class="mt-1 p-2 bg-gray-50 rounded">{{ detailData.readOrSent ? '是' : '否' }}</div>
          </div>
        </div>
...
import { ref, reactive, computed, onMounted, watch, h, defineAsyncComponent } from 'vue';
...
const OperationMessageFormModal = defineAsyncComponent(() => import('./components/OperationMessageFormModal.vue'));
const OperationMessagePMD = defineAsyncComponent(() => import('./components/OperationMessagePMD.vue'));
const OperationMessageGG = defineAsyncComponent(() => import('./components/OperationMessageGG.vue'));
const LobbyBannerManager = defineAsyncComponent(() => import('./components/LobbyBannerManager.vue'));
const LobbyPopModalManager = defineAsyncComponent(() => import('./components/LobbyPopModalManager.vue'));
import {
...
const columns: DataTableColumns<OperationMessage> = [
  ...
  },
  {
    ...
    render(row) {
      ...
      const status = statusMap[row.status] || { label: row.status, type: 'error' };
      return h(NTag, {
        type: status.type,
        size: 'small'
...
const handleBatchSend = async (selectedRows?: OperationMessage[]) => {
  const messagesToSend = selectedRows || tableData.value.filter(message => 
...
const handleBatchDelete = async (selectedRows?: OperationMessage[]) => {
  const messagesToDelete = selectedRows || tableData.value.filter(message => 
    selectedRowKeys.value.includes(message.id)
  );
...
const initializeTab = () => {
  ...
  if (activeName && ['operation_message_tz', 'operation_message_pmd', 'operation_message_gg', 'lobby_banner', 'lobby_pop'].includes(activeName)) {
    activeTab.value = activeName;
  }
};

// Watch for route changes
watch(() => route.query.activeName, (newActiveName) => {
  try {
    if (newActiveName && ['operation_message_tz', 'operation_message_pmd', 'operation_message_gg', 'lobby_banner', 'lobby_pop'].includes(newActiveName as string)) {
