      ...
      <n-scrollbar style="max-height: calc(90vh - 200px);">
        ...
        >
          ...
          <n-form-item label="未开通节点">
            <div class="cdn-nodes-container">
              ...
              >
                ...
                <!-- Activate Button -->
                <n-button size="small" type="primary" ghost @click="handleActivateNode(node)">
                  开通
                </n-button>
              </div>
            </div>
            <div style="text-align: center; margin-top: 12px;">
              <n-button text type="primary" @click="showMoreNodes = !showMoreNodes">
                {{ showMoreNodes ? '收起' : '更多' }}
                <n-icon :component="showMoreNodes ? ChevronUp : ChevronDown" style="margin-left: 4px;" />
              </n-button>
            </div>
          </n-form-item>
          ...
          <n-form-item path="domains">
            ...
            />
            <div v-if="domainsError" style="color: #d03050; font-size: 12px; margin-top: 4px;">
              {{ domainsError }}
            </div>
          </n-form-item>
          ...
          <div class="tutorial-section">
            ...
            <div class="tutorial-item">
              ...
              <div class="tutorial-content">
                ...
                <n-button text type="primary" size="small" @click="openTutorial('domain')">域名管理教程</n-button>
              </div>
            </div>
            ...
            <div class="tutorial-item">
              ...
              <div class="tutorial-content">
                <div class="tutorial-subsection">
                  <strong>(1) 中国市场 (含港澳台)：</strong> 推荐使用alb域名 (CloudFront-ALB)、三方公共域名 (CloudFront防封)、Wangsu (CDN+IP)、Apeir8、Yundun (云盾)、Funnull (方能)、superEdge、大陆阿里云。
                </div>
                <div class="tutorial-subsection">
                  <strong>(2) 全球市场 (即非中国)：</strong> 推荐使用alb域名 (CloudFront-ALB与阿里云ALB)、三方公共域名 (CloudFront防封与Azure防封)、CloudFront、Cloudflare、腾讯云、阿里云、Google、Azure、华为云、Wangsu (CDN+IP)、Yundun (云盾)。
                </div>
                <div class="tutorial-subsection">
                  <strong>(3) Wangsu独立IP：</strong> 提供独立IP地址，适用于全球禁域名，但不会封禁IP的国家（例如：印尼、中国、巴西、越南等），国际和中国大陆市场均可使用（即全球通用130USDT/IP/月），若超过7天未启用系统自动回收。
                </div>
                <div class="tutorial-subsection">
                  <strong>(4) 开放回源：</strong> 我方提供独立源服务器IP信息，您可使用外部三方CDN绑定到我方源站，所以仅限web大厅使用（不支持原生APP)，需要有专业知识才能配置websocket、HTTP2、缓存等信息，详见域名管理教程。
                </div>
              </div>
            </div>
          </div>
...
const validateDomains = () => {
  ...
  const domains = formData.value.domains.trim().split('\n').filter(d => d.trim());
...
const handleSubmit = async () => {
  try {
    ...
    const results = await Promise.allSettled(
      domains.map(async domain => {
        const response: any = await domainApi.createDomain({
          ...
          cdnPlatformName: activatedNodes.find(n => n.value === formData.value.cdnProvider)?.label || '',
        ...
        if (code === 0) {
          ...
          throw new Error(response.message || response.data?.message || 'Creation failed');
        }
      })
    );
    ...
    if (successful.length > 0 && failed.length === 0) {
      ...
      // Partial success
      const failedDomains = failed.map((r: any) => r.reason?.message || '未知错误');
      message.warning(
