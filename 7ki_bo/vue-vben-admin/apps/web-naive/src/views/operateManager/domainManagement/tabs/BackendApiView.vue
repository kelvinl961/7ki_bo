<template>
  <div class="backend-api-view">
    ...
    <!-- Create Backend API Domain Modal (Custom) -->
    <BackendApiCreateModal
      v-model:show="showCreateModal"
      use-type="BACKEND_API"
      :use-type-label="$t('operations.domain.useType.backendApi')"
      :modal-title="$t('operations.domain.createBackendApiSubdomain')"
      ...
      <n-space vertical :size="16" style="margin: 20px 0">
        ...
        <div>
          <div style="margin-bottom: 8px; font-weight: 500">
            {{ $t('operations.layout.agent') }}ID <span style="color: red">*</span>
          </div>
          <n-input v-model:value="agentId" :placeholder="$t('operations.domain.modal.agentIdPlaceholder')" />
        </div>
      </n-space>
    </n-modal>
  </div>
...
const columns: DataTableColumn<Domain>[] = [
  ...
  {
    title: $t('operations.domain.useType.backendApi'),
    key: 'domainName',
    width: 250,
  ...
  {
    title: $t('operations.domain.column.sslCert'),
    key: 'sslEnabled',
    width: 100,
    render(row: Domain) {
      return h(
  ...
  {
    title: $t('operations.domain.column.verifyStatus'),
  ...
  },
  {
    ...
    render(row: Domain) {
      const statusMap: Record<string, { text: string; type: any }> = {
        NORMAL: { text: $t('operations.domain.status.normal'), type: 'success' },
        DISABLED: { text: $t('operations.messageSettings.option.statusDisabled'), type: 'error' },
        EXPIRED: { text: $t('operations.domain.status.expired'), type: 'warning' },
        EXPIRING_SOON: { text: $t('operations.domain.status.expiringSoon'), type: 'warning' },
        VERIFICATION_PENDING: { text: $t('operations.domain.status.verificationPending'), type: 'info' },
      };
  ...
  {
    title: $t('operations.domain.action.bindAgent'),
    key: 'boundAgentId',
    width: 120,
    render(row: Domain) {
