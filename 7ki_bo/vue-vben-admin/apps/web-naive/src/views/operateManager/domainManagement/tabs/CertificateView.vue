    ...
    >
      ...
      >
        <n-form-item label="类型" path="type">
          <n-radio-group v-model:value="applyForm.type">
            ...
            <n-radio value="wildcard">多域名通配符证书 (仅支持顶级域名)</n-radio>
          </n-radio-group>
        </n-form-item>
        ...
        <n-form-item label="*域名" path="domains">
          ...
          />
          <div v-if="!applyForm.domains" style="color: #d03050; font-size: 12px; margin-top: 4px;">
            格式不正确
          </div>
        </n-form-item>
      ...
      <template #footer>
        <n-space justify="end">
          ...
          <n-button type="primary" @click="handleApplyCertificate" :loading="applying">
            申请
          </n-button>
        </n-space>
      </template>
    ...
    >
      ...
      <template #footer>
        <n-space justify="end">
          ...
          <n-button type="primary" @click="handleUploadCertificate" :loading="uploading">
            上传
          </n-button>
        </n-space>
      </template>
    ...
    >
      ...
      >
        ...
        <n-descriptions-item label="签发日期">
          {{ selectedCertificate.issuedAt ? new Date(selectedCertificate.issuedAt).toLocaleDateString('zh-CN') : '--' }}
        </n-descriptions-item>
        <n-descriptions-item label="到期日期" :span="2">
          {{ selectedCertificate.expiresAt ? new Date(selectedCertificate.expiresAt).toLocaleDateString('zh-CN') : '--' }}
        </n-descriptions-item>
...
const uploadRules: FormRules = {
  ...
  crtContent: [{ required: true, message: '请输入CRT文件内容', trigger: 'blur' }],
  keyContent: [{ required: true, message: '请输入KEY文件内容', trigger: 'blur' }]
};
...
const columns: DataTableColumn<Certificate>[] = [
  ...
  { 
    title: () => h('div', {}, [
      h('span', {}, '验证方式 '),
      h(NTooltip, {}, {
        trigger: () => h('span', { style: 'color: #2080f0; cursor: help;' }, 'ⓘ'),
  ...
  { 
    ...
    render(row: Certificate) {
      return h('div', { style: 'display: flex; gap: 4px; flex-wrap: wrap;' }, [
        ...
        row.status === 'EXPIRING_SOON' || row.status === 'EXPIRED' ? h(NButton, {
          text: true,
          type: 'warning',
          size: 'small',
        ...
        row.provider === 'AWS_ACM' && row.status === 'PENDING_VALIDATION' ? h(NButton, {
          text: true,
          type: 'success',
          size: 'small',
          onClick: () => handleValidateDNS(row)
        }, { default: () => '验证DNS' }) : null,
        row.provider === 'AWS_ACM' && row.status === 'ISSUED' ? h(NButton, {
          text: true,
          type: 'warning',
          size: 'small',
          onClick: () => handleConfigureNGINX(row)
        }, { default: () => '配置HTTPS' }) : null,
        h(NPopconfirm, {
          ...
          default: () => h('div', { style: 'font-size: 14px; padding: 10px 0;' }, [
            h('div', { style: 'font-weight: 600; margin-bottom: 8px; font-size: 16px;' }, '⚠️ 确认删除证书？'),
            h('div', { style: 'color: #666;' }, `证书域名: ${row.domains?.join(', ') || '未知'}`),
...
const handleApplyCertificate = async () => {
  ...
  try {
    ...
    const domainLookupResponse = await fetch(`/api/domain-management?search=${encodeURIComponent(mainDomain || '')}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('token')}`
    ...
    if (domainLookupResult.code !== 0 || !domainLookupResult.data?.list || domainLookupResult.data.list.length === 0) {
      message.error(`域名 ${mainDomain} 未在系统中找到，请先在域名管理中创建该域名`);
      return;
    }
    
    // Find exact match (search might return partial matches)
    const domainRecord = domainLookupResult.data.list.find((d: any) => d.domainName === mainDomain);
    ...
    if (result.success) {
      message.success(`✅ ${mainDomain} 证书已创建！状态：待验证。请在证书列表中查看详情和安装脚本。`);
...
const handleConfigureNGINX = async (cert: Certificate) => {
  try {
    ...
    const response = await fetch(`/api/aws-ssl/configure-nginx/${cert.domainId}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  try {
    ...
    if (result.success) {
      ...
      if (result.validationRecord) {
        message.info(`DNS记录: ${result.validationRecord.name} = ${result.validationRecord.value}`);
      }
      fetchCertificates();
    } else {
