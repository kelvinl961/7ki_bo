      ...
      <n-card size="small" :bordered="false">
        <div style="margin-bottom: 16px;">
          <n-space align="center">
            <span style="font-size: 14px; font-weight: 500;">月份：{{ selectedMonth }}</span>
            <span style="font-size: 14px; font-weight: 500;">子域名数量：{{ selectedDomainCount }}</span>
          </n-space>
        </div>
  ...
  try {
    ...
    if (result.code === 0 && result.data) {
      ...
      sortedMonths.forEach(([monthKey, data]) => {
        ...
        const excessUnitPrice = 0.20; // From config
        ...
        const domainUnitPrice = 3.00; // From config
...
const handleShowDomainDetail = async (row: TrafficStats) => {
  ...
  try {
    ...
    if (result.code === 0 && result.data) {
      ...
      const formattedDomains: DomainDetail[] = Array.from(domainTrafficMap.values()).map(d => ({
        ...d,
        traffic: Number(d.traffic.toFixed(2)),
        status: d.status === 'NORMAL' ? '正常' : 
                d.status === 'DISABLED' ? '停用' : 
                d.status === 'EXPIRED' ? '已过期' : '异常'
      }));
