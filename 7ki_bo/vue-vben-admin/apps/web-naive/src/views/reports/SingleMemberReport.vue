<template>
  <div class="single-member-report">
    <n-card :title="$t('reports.singleMemberReport')" class="mb-4">
      <div class="mb-4">
        <n-form inline>
          <n-form-item :label="$t('common.memberAccount')">
            <n-input
              v-model:value="memberAccount"
              :placeholder="$t('reports.enterMemberAccount')"
              style="width: 200px"
            />
          </n-form-item>
      ...
      <div v-if="reportData" class="py-8 text-center">
        <n-alert type="info" :title="$t('reports.singleMemberReportDev')" />
...
const fetchData = async () => {
  if (!memberAccount.value) {
    message.warning($t('reports.enterMemberAccount'));
    return;
  }
