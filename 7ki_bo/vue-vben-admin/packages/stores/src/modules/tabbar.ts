...
function getTabKey(tab: RouteLocationNormalized | RouteRecordNormalized) {
  ...
  if (pageKey) {
    ...
    // Use path when fullPathKey is explicitly set to false
    rawKey = path;
  } else {
    // Use fullPath, but normalize it to remove trailing slashes and duplicates
    const normalizedPath = (fullPath ?? path).replace(/\/+$/, ''); // Remove trailing slashes
    // If the normalized path is empty or just '/', use the route name as key to prevent duplicates
    rawKey = normalizedPath || (name as string);
  }
  try {
